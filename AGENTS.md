...
- Deployable sites live as Sternsystemen discovered via convention-based per-system files in `systems-cache/{id}/` (RFC-0790): `system-config.yaml` (static config) and `system-state.yaml` (mutable state). The `systems-cache/` directory is a **sibling of the werkstatt repo**, not inside it — `mirrors[0].path` in `system-config.yaml` is relative to the werkstatt repo's parent (e.g. `../systems-cache/<id>` resolves to `<werkstatt-parent>/systems-cache/<id>`). Each Sternsystem declares a `mirrors[]` array (RFC-0574): `mirrors[0]` is the non-bare cache clone (outside the monorepo), `mirrors[1]` is the bare repo, and `mirrors[2+]` are external mirrors. A pin file lives at `mirrors[0].path/system.pin.json`, and an active mission workpiece at `missions/<missionId>/workpiece/` when a mission is in progress. The `apps/*` directory is retired (RFC-0381); shared site composition conventions now live in `docs/authoring/site-composition.md`.
...
- New workshops (consumer monorepos) must be created via `forge create --in-place --profile <profile-id>` (RFC-0877), not by copying this repository. The command scaffolds directly into the current directory with `tools/kernel.config.ts`, `forge.yaml`, `.npmrc`, CI, stack-specific customization, and delegates forge artifacts to `forge.init`. Available stack profiles: `astro-typescript-turborepo` (site), `phaser-turborepo` (game), `godot-csharp` (Godot), `forge-shell` (minimal).
...
- `mission.close` also calls `sternsystem.sync` automatically (RFC-0762, RFC-0797): `sternsystem.sync` is called **twice** during close — once inside the lock before the mirror sync check (RFC-0797, to update `refs/mirror` after inline validate's bordbuch commits, preventing false "out of sync" errors), and once after all cache clone commits (werkstatt, pin, bordbuch) and before writing `.materialization-state.json` (RFC-0762, to propagate close-created commits to external mirrors). Both calls are non-fatal (`logger.warn`). The pre-check sync can be disabled with `--skip-auto-sync`. The `closeReport.mirror.synced` and `closeReport.mirror.syncError` fields track the post-close sync result.
...
- Agents MUST NEVER edit any Sternsystem mirror directly — only through mission workpieces. This rule is a **soft guard** — it relies on agent compliance. The **hard guard** is the cache-clone pre-commit hook (installed by `installBordbuchPreCommitHook`, RFC-0658 + RFC-0821) that blocks `git commit` in cache clones unless `MISSION_GIT_COMMIT=1` is set. Agents MUST NOT use `git commit --no-verify` to bypass this hook.
- **Destructive Sternsystem ID migration** (e.g. TLD removal per RFC-0902): rename `systems-cache/{old-id}/` and `systems-git/{old-id}/` directories to the new ID — do NOT delete and re-register. Renaming preserves git history, bordbuch, releases, and `.env*` files. The migration MUST go through a mission workpiece — agents cannot edit systems-cache files directly, even for operator-task migrations. Open a mission, make changes through the workpiece, then close. Cloudflare Worker names (in `deployment.channels`) are Cloudflare resource names, not Sternsystem IDs — do NOT rename them during ID migration.
...
- **Deployment** (RFC-0865, RFC-0866, RFC-0895): `leitstand.certify`, `leitstand.dev-deploy`, `leitstand.propagate`, `leitstand.promote`, `leitstand.status`, `leitstand.health`, `leitstand.pipeline.check`, `leitstand.rollback` — all work via certification authority (`authorizeDeployment()`, `verifyMainPromotion()`). `leitstand.certify` produces `GateDecisionV1` JSON at `systems-cache/{id}/gate-decisions/{release}-{gate}.json` from certification orchestration. Deploy commands resolve the gate decision at the conventional path by default; `--gate-decision` overrides the path. `leitstand.promote` additionally requires `--main-verification-decision`. The 13-phase deploy execution pipeline (build, wrangler deploy, build-identity, CDN purge, freshness, health, mission.check, Axiom gate, alt health check, evidence sync, bordbuch, system-state, effect record) runs after authorization. `leitstand.rollback` (RFC-0895) uses native `wrangler rollback` and accepts `--site` or `--service` (mutually exclusive); `--gate-decision` and `--to-release` are no longer required for rollback.
- **Access protection** (RFC-0899): `leitstand.access.protect`, `leitstand.access.unprotect`, `leitstand.access.status` — manage 4-digit PIN Basic Auth for dev/alt subdomains. `protect` sets `ACCESS_PIN` secret on dev+alt Workers via `wrangler secret put` and records PIN in `system-state.yaml`. `unprotect` removes the secret. `status` reports protection state. Middleware runs first in chain, before tombstones/redirect/negotiation.
...
- All Leitstand commands (`leitstand.dev-deploy`, `leitstand.propagate`, `leitstand.promote`, `leitstand.status`, `leitstand.rollback`, `leitstand.health`, `leitstand.pipeline.check`) are now **unblocked** (RFC-0865). They call `authorizeDeployment()` and `verifyMainPromotion()` from `packages/werkstatt/src/certification/deployment/authority.ts` via `packages/werkstatt/src/leitstand/deploy-helpers.ts`. Each deploy command requires a `--gate-decision` flag pointing to a `GateDecisionV1` JSON file; `leitstand.promote` additionally requires `--main-verification-decision`. Deployment effect records are written to `systems-cache/{id}/deployment-operations/`. `leitstand.rollback` (RFC-0895) is exempt from `--gate-decision` — it uses native `wrangler rollback` and accepts `--site` or `--service`. `release.rollback` and `leitstand.service.rollback` are removed (RFC-0895).
...
- Env-and-deploy contract (RFC-0761 / DNA-40) — .env.example, .env, deploy.preflight, # How to obtain: instructions; `null` marker for not-required variables (RFC-0819)
...
- `mission.close` enforces a reconcile-freshness gate (RFC-0913): it compares the workpiece HEAD against `workpieceHeadAtReconcile` from `evidence/reconciliation-report.json`. If they differ, close blocks with the count of unreconciled commits. If the report is missing or unreadable, close fails closed. The `--skip-reconcile-check` flag is an escape hatch for edge cases only — it appends a Bordbuch audit entry. Agents MUST NOT use `--skip-reconcile-check` unless explicitly directed by the operator.
- `mission.reconcile` restores cache-clone-only `.gitignore` patterns after merge (RFC-0913): if the merge overwrites `.gitignore` with the workpiece version, cache-clone-only entries (forbidden files, generated files) are re-appended. Forbidden/generated files that were re-tracked by the merge are untracked via `git rm --cached`. The `reconciliation-report.json` includes `workpieceHeadAtReconcile`, `gitignoreRestored`, and `forbiddenFilesUntracked` fields.
...
- **Axiom methodology activation is config-only.** To activate or deactivate an Axiom methodology, change `active: true`/`false` in `systems/methodologies.md`. No code changes, no CLI changes, no new commands. `mapMethodologiesConfig` in `axiom-adapter.ts` is generic and picks up the change automatically. Commit via `ecosystem.commit`. Verify with `methodologies.validate`.
