...
| `packages/os/site-kernel-checks/src/command-tables/` | Remove `contact.form.validate` from command table entries |
| `packages/os/site-kernel-handoff/src/migrators/rfc-0572.ts` | New migrator — strips `emailField`/`phoneField` props, re-adds `contactRequirementMessage` |
...
- **Migrator (RFC-0479):** A migrator (`rfc-0572.ts`) runs during `mission.migrate` for each site with a `send-message` block. It removes `emailField` and `phoneField` props from the block configuration and re-adds `contactRequirementMessage` if not present. The migrator also removes `emailFieldLabel`, `phoneFieldLabel`, `emailFieldPlaceholder`, `phoneFieldPlaceholder` from site labels if they exist.
...
- **Regex fragility.** International phone formats, multiline inputs, and edge cases can cause missed or incorrect extraction. Mitigation: the `contactRequirementMessage` hint tells the visitor to include email or phone, and the form does not submit until contact details are detected. The visitor can adjust and resubmit.
