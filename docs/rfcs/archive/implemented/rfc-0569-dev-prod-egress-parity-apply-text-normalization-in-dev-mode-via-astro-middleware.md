...
- [x] Running `routes.generate --site warpgogol-com` regenerates `src/middleware.ts` with the dev-normalize middleware chained. (evidence: missions/warpgogol-com-m000016/workpiece/src/middleware.ts:20-30, `pnpm exec werkstatt run routes.generate --site warpgogol-com`)
...
- [x] `rfc.validate` passes on this file. (evidence: `pnpm exec werkstatt run rfc.validate --json` — no violations for RFC-0569)
