...
- [x] An AMD-007 draft exists with exact proposed component-set/evaluator/roadmap changes, no immutable snapshot file is edited, and `spec.validate --spec=werkstatt-release-certification --json` passes; acceptance remains a separate explicit operator decision before packet 040 completion. (evidence: `docs/specs/werkstatt-release-certification/amendments/amd-007-component-runtime-reconciliation.md`; review mechanical evidence)
- [x] RFC-0769 and RFC-0770 are formally superseded, and the successor DNA-64 wording preserves the stack-agnostic engine/profile inversion while replacing the one-plugin clause with the resolved-component-graph boundary. (evidence: `docs/rfcs/archive/superseded/rfc-0769-*.md`; `docs/rfcs/archive/superseded/rfc-0770-*.md`; `docs/architecture-dna.md` DNA-64)
- [x] RFC-0848, RFC-0850, RFC-0851, and RFC-0853 are enhanced against RFC-0855/AMD-007 without duplicating RFC-0854, RFC-0849, or RFC-0852. (evidence: retained RFC frontmatter/context and `docs/audits/audit-rfc-0848-*.md`, `audit-rfc-0850-*.md`, `audit-rfc-0851-*.md`, `audit-rfc-0853-*.md`)
- [x] RFC-0856 exists in `accepted`; every other new non-spec child RFC required by packets 050–200 exists in `draft`; all program children use `batch: agent-runtime-certification-program`, declare direct `dependsOn`, and have no unresolved duplicate responsibility. (evidence: RFC-0856 and RFC-0858 through RFC-0864 frontmatter; approved child audit files)
- [x] `program.yaml`, `README.md`, `packet-template.md`, every draft packet, completion template, and recovery template exist with exact sequential order and no parallel path. (evidence: `docs/plans/agent-runtime-certification/`; `PROGRAM-DOC-READY 25`)
- [x] Every draft packet contains decision prerequisites, normative source hashes, required reads, current code facts, allowed/forbidden files, ordered steps, exact expected validations, permitted transition diagnostics, recovery, and handoff fields; future `baseCommit` is schema-valid `null` until just-in-time sealing. (evidence: all 25 packet frontmatters and ten-section bodies; readiness validator)
- [x] No packet contains `NEEDS CLARIFICATION`, a prose placeholder, an unbounded file scope, permission to update its own allow-list, a compatibility path, or production agent-written activation before packets 190–200. (evidence: bounded-scope and placeholder checks in `docs/reviews/code/docs/review-2026-08-15-rfc-0855-document-readiness.md`)
- [x] Packet 000 documents the sole accepted-plan bootstrap; packets 010–240 require RFC-0856 validation, distinct Steward/Executor identities, exact seal/implementation/completion ancestry, and an exclusive lease. (evidence: `docs/plans/agent-runtime-certification/000-program-control-plane.md`; common packet sections 2 and 10)
- [x] Root and package AGENTS guidance records full sequential execution, accepted operational unavailability, bounded transition diagnostics, Law Kernel ownership, and no self-authorization. (evidence: `AGENTS.md`; `packages/AGENTS.md`; Forge, Werkstatt, site, game, and video package guides)
- [x] Relevant requirements, technology, development-plan, knowledge-graph, verification-plan, and source-markup Compass contracts describe the program; styling records a reviewed no-change rationale rather than unrelated edits. (evidence: changed root Compass XML; styling no-change rationale in the documentation readiness review)
- [x] `rfc.validate` passes for RFC-0855 and every child draft, `spec.validate` passes, all referenced RFC dependencies resolve, draft program artifacts validate against their checked-in schemas or pre-implementation fixtures, and `bash scripts/check-clean-trees.sh` reports clean trees. (evidence: targeted validation run; `RFC-DEPENDENCIES-RESOLVE 14`; `PROGRAM-DOC-READY 25`; final clean-tree gate)
