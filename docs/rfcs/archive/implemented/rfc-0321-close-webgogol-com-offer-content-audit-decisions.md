---
id: RFC-0321
title: "Close webgogol-com offer content audit decisions"
status: implemented
kind: contract
scope: app
owners:
...
successSignals:
  - "The German webgogol-com offer surface has one response-time guarantee: 24 hours on working days."
...
acceptance:
  - probe: run
    command: "site-kernel run content.references.validate --app webgogol-com --json"
    expect:
      exitCode: 0
  - probe: run
    command: "site-kernel run redirect.map.validate --app webgogol-com --json"
    expect:
      exitCode: 0
  - probe: run
...
# RFC-0321: Close webgogol-com offer content audit decisions

## Context

The July 5 public Markdown audit reviewed `apps/webgogol-com/public/`, not the full source tree. It still surfaced real product defects because public twins, `llms-full.txt`, feed output, sitemap membership, and agent knowledge are projections of the source content.
...
This RFC turns those decisions into a bounded implementation contract for `webgogol-com`. It is app-scoped because several decisions are studio-specific, but the implementation must use shared data and generator paths where the same class of defect can recur.
...
- generic public hygiene gaps are covered by RFC-0316, not by webgogol-specific command names.
...
This RFC deliberately does not create commands such as `webgogol.response-copy.validate`.
...
- Do not create permanent validators that only know webgogol-com phrases.
