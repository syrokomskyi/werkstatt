...
- The validator MUST use the same `resolveIconFileName` logic as `icon-resolver.ts` to ensure build-time and runtime resolution are consistent. `resolveIconFileName` is extracted into `resolve-icon-file-name.ts` (Node.js-safe, no `import.meta.glob`) and re-exported from `icon-resolver.ts`. Import from `../domain/ui/icons/resolve-icon-file-name.ts` in Node.js contexts, or from `@warpgogol/werkstatt-site/ui/icons` in Astro/client contexts.
