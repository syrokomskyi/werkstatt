...
pnpm exec werkstatt run gitattributes.generate --json

# Fail if the managed block is stale/unsorted; warn on uncovered marked files (read-only).
pnpm exec werkstatt run gitattributes.validate
pnpm exec werkstatt run gitattributes.validate --json

# Fail if a generated file was hand-edited without its generator (VCS-aware, read-only).
pnpm exec werkstatt run generated.edit.guard
pnpm exec werkstatt run generated.edit.guard --range origin/main..HEAD --json
```
...
`GITATTR-01` fixHint: `Run pnpm exec werkstatt run gitattributes.generate`.
...
<!--
  ...
  Regenerate:   pnpm exec werkstatt run routes.generate --app <app>
-->
```
...
| `regenerateCommand` | Exact CLI command to regenerate, e.g. `"pnpm exec werkstatt run <command>"` or `"... --app <app>"`. |
