...
packagesImpacted:
  ...
  - "@gogol/ontology"
successSignals:
  - "warpgogol-com is registered as a Sternsystem in systems/registry.yaml with repo pointing to ../systems-git/warpgogol-com"
  - "sternsystem.extract --app warpgogol-com --repo ../systems-git/warpgogol-com copies data-only paths (src/content, public, provenance) into systems/warpgogol-com/ and writes system.pin.json"
  - "sternsystem.validate --id warpgogol-com passes after extraction"
  - "A mission is opened, materialized, validated, built, reconciled, and closed for warpgogol-com, producing a validated distribution"
  - "release.prepare --mission warpgogol-com-m000001 produces release candidate warpgogol-com-r000001 with behavior snapshots"
  - "release.publish --release warpgogol-com-r000001 publishes with artifact store reference and release manifest"
  - "leitstand.propagate --release warpgogol-com-r000001 --channel alt deploys to alt-warpgogol-com worker"
  - "Health check verifies alt deployment against behavior snapshot fingerprint"
  - "leitstand.propagate --release warpgogol-com-r000001 --channel main deploys to production warpgogol-com worker"
  - "notausgang.export --system warpgogol-com --release warpgogol-com-r000001 produces a valid export package"
  - "apps/warpgogol-com/ directory is removed from the monorepo after extraction validates"
  - "pnpm-workspace.yaml no longer includes apps/* glob after pilot is complete"
  - "fleet.sites.generate resolves warpgogol-com from systems/registry.yaml, not apps/ discovery"
nonGoals:
  - "Does not migrate sites other than warpgogol-com — other sites were already removed and will be re-onboarded individually as Sternsystems"
...
The `apps/` directory currently contains only `warpgogol-com` (other sites were removed earlier). This is the pilot candidate. The operator has decided:

1. **Git hosting:** Local bare repo at `../systems-git/warpgogol-com` (relative to workspace root). GitHub hosting is a later step.
2. **Propagation order:** Alt channel first (`alt-warpgogol-com` → alt.warpgogol.com), health-verify, then main channel (`warpgogol-com` → production).
...
2. **`apps/` still contains a live site (DNA-44):** The Sternsystem contract states that after extraction validates, the source `apps/<id>` workspace is removed. `apps/warpgogol-com/` still exists, meaning the platform has not completed the migration it designed.
3. **No fleet projection from registry (RFC-0378):** `fleet.sites.yaml` is still hand-authored pointing at `apps/warpgogol-com`. It should be generated from `systems/registry.yaml`.
4. **No deployment proof (DNA-49, RFC-0379):** The Cloudflare Workers adapter has never propagated a real release to `alt-warpgogol-com` or `warpgogol-com`.
...
Execute the pilot migration wave for `warpgogol-com`: extract it into a Sternsystem, run a full mission cycle, publish release `warpgogol-com-r000001`, propagate to alt then main via the Cloudflare Workers Leitstand adapter, verify health, generate and validate a Notausgang export, and retire `apps/warpgogol-com/` from the monorepo.
...
- Local path: `../systems-git/warpgogol-com` or `/absolute/path/to/repo`
...
- **DNA-45 (Fleet registry):** The pilot registers `warpgogol-com` in `systems/registry.yaml` with a real `repo` path and `deployment` config.
- **DNA-46 (Mission lifecycle):** The pilot opens, materializes, builds, and closes a real mission for `warpgogol-com`.
...
- **DNA-48 (Release discipline):** The pilot prepares and publishes release `warpgogol-com-r000001` with behavior snapshots, artifact store reference, and a valid release manifest.
- **DNA-49 (Fleet propagation):** The pilot propagates `warpgogol-com-r000001` to `alt-warpgogol-com` then `warpgogol-com` via the Cloudflare Workers adapter, with health verification.
- **DNA-50 (Notausgang export):** The pilot generates and validates a Notausgang export from `warpgogol-com-r000001`.
...
```sh
git init --bare ../systems-git/warpgogol-com
```
...
Copies `src/content/`, `public/`, `provenance/` from `apps/warpgogol-com/` into `systems/warpgogol-com/`. Writes `system.pin.json` with current platform version, commit, RFC head, and `platformSemanticHash`. Appends initial Bordbuch `pin-update` entry. Updates registry status to `active`.
...
Creates mission `warpgogol-com-m000001` in `missions/`. Acquires system lock. Appends Bordbuch `mission-open` event.
...
Materializes `missions/warpgogol-com-m000001/workpiece/` from the pinned Sternsystem data bundle and generated runtime scaffolding.
...
Produces `missions/warpgogol-com-m000001/distribution/dist/`.
...
Produces release candidate `warpgogol-com-r000001` in `releases/warpgogol-com-r000001/`. Runs a production build, captures behavior snapshots (readable + production), runs `behavior.snapshot.diff`, and writes `release.yaml` with state `prepared`.
...
Deploys to `alt-warpgogol-com` worker via `wrangler deploy`. Health-checks alt.warpgogol.com against the behavior snapshot fingerprint.
...
Deploys to `warpgogol-com` worker via `wrangler deploy`. Health-checks warpgogol.com against the behavior snapshot fingerprint. Refuses unless `deployment.lastPropagated.alt` records the same release with `healthy: true` (RFC-0379 channel gating).
...
| Path | Role |
| --- | --- |
| `apps/warpgogol-com/` | Source for extraction, then removed |
| `systems/warpgogol-com/` | Sternsystem data bundle (content, public, provenance, `system.pin.json`, bordbuch) |
| `systems/registry.yaml` | Fleet registry with warpgogol-com entry |
| `missions/warpgogol-com-m000001/` | Ephemeral mission workspace |
| `releases/warpgogol-com-r000001/` | Published release with artifact reference |
| `fleet/fleet.sites.yaml` | Generated projection, no longer hand-authored |
| `pnpm-workspace.yaml` | `apps/*` glob removed after pilot |
| `../systems-git/warpgogol-com` | Local bare git repo for Sternsystem data |

### Failure modes

- **Extraction fails:** Abort pilot. `apps/warpgogol-com/` remains intact. Registry entry stays `registered`. Re-run `sternsystem.extract` after fixing the data classifier or platform templates. The extraction is idempotent under RFC-0362 operation records.
- **Materialization fails:** Abort pilot. Mission is aborted via `mission.abort --mission warpgogol-com-m000001`. `apps/` remains intact. Re-open a new mission after fixing the materialization issue; mission IDs are sequential so the next attempt is `m000002`.
...
- **Alt propagation fails:** Abort pilot. Main channel is NOT deployed. Alt worker may have a partial deploy — rollback via `leitstand.rollback --system warpgogol-com --channel alt`. The RFC-0362 operation record and Bordbuch entry capture the failure.
- **Alt health check fails:** Do NOT propagate to main. Investigate health discrepancy. Rollback alt if needed via `leitstand.rollback --system warpgogol-com --channel alt`. The `lastPropagated.alt.healthy: false` state blocks main promotion automatically.
- **Main propagation fails:** Production is NOT affected. Rollback main via `leitstand.rollback --system warpgogol-com --channel main`.
- **Main health check fails:** Rollback main immediately via `leitstand.rollback --system warpgogol-com --channel main`. Alt remains deployed (can serve as fallback if DNS is pointed).
...
- **apps/ removal after extraction:** Only proceed if `sternsystem.validate --id warpgogol-com` passes AND `notausgang.validate` passes. If either fails, keep `apps/` and investigate.
...
- **Pilot only:** This RFC covers only `warpgogol-com`. Other sites will be re-onboarded individually as Sternsystems in follow-up RFCs.
- **Forward-only:** Once `apps/warpgogol-com/` is removed, there is no rollback path to the `apps/` layout. The Notausgang export and the Sternsystem git repo are the safety net.
...
- **CI pipeline update:** CI workflows that reference `apps/warpgogol-com` paths are updated to resolve via `fleet.sites.yaml` or `systems/registry.yaml`.
...
- **Run multiple sites in the pilot:** Rejected — `warpgogol-com` is the only remaining site in `apps/`. A single-site pilot is the minimal viable proof.
...
- **Agent misinterpretation:** Agents may attempt to re-create `apps/warpgogol-com/` after removal. Mitigation: `fleet.sites.generate` and `workspace.surface.validate` will fail if `apps/` is referenced but does not exist.

## Acceptance criteria

- [x] `systems/registry.yaml` contains `warpgogol-com` with `status: active`, `repo: ../systems-git/warpgogol-com`, and `deployment.adapter: cloudflare-workers` (evidence: implemented historically)
- [x] `systems/warpgogol-com/` contains `src/content/`, `public/`, `provenance/`, `system.pin.json`, and `bordbuch/events.ndjson` (evidence: implemented historically)
- [x] `sternsystem.validate --id warpgogol-com` passes (evidence: implemented historically)
- [x] Mission `warpgogol-com-m000001` is opened, materialized, validated, built, reconciled, and closed (evidence: implemented historically)
- [x] `release.prepare --mission warpgogol-com-m000001` produces release candidate `warpgogol-com-r000001` with behavior snapshots (evidence: implemented historically)
- [x] `release.publish --release warpgogol-com-r000001` publishes with artifact store reference (evidence: implemented historically)
- [x] `release.validate --release warpgogol-com-r000001` passes (evidence: implemented historically)
- [x] `leitstand.propagate --release warpgogol-com-r000001 --channel alt` succeeds and health check passes (evidence: implemented historically)
- [x] `leitstand.propagate --release warpgogol-com-r000001 --channel main` succeeds and health check passes (evidence: implemented historically)
- [x] `leitstand.status --system warpgogol-com` shows both channels deployed at `warpgogol-com-r000001` (evidence: implemented historically)
- [x] `notausgang.export` and `notausgang.validate` pass for `warpgogol-com-r000001` (evidence: implemented historically)
- [x] `apps/warpgogol-com/` directory is removed from the monorepo (evidence: implemented historically)
- [x] `pnpm-workspace.yaml` no longer includes `apps/*` or `apps/*/workers/*` globs (evidence: implemented historically)
- [x] `fleet.sites.generate` resolves `warpgogol-com` from `systems/registry.yaml` (evidence: implemented historically)
...
- Agents MUST NOT remove `apps/warpgogol-com/` until `sternsystem.validate` AND `notausgang.validate` both pass.
...
- Agents MUST update `pnpm-workspace.yaml` and run `pnpm install` in the same commit that removes `apps/warpgogol-com/`.
- Agents MUST push the Sternsystem data to `../systems-git/warpgogol-com` after extraction so the bare repo has content.
- Agents MUST update CI workflow files that reference `apps/warpgogol-com` paths to resolve via `fleet.sites.yaml` or `systems/registry.yaml`.
