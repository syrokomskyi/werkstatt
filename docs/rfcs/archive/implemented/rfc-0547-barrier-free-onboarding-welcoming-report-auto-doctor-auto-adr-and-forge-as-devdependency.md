...
successSignals:
  ...
  - "Scaffold profiles add @webgogol/forge as devDependency so forge commands work inside the new project"
...
Additionally, `forge doctor` and other forge CLI commands do not work inside a newly created forge project because scaffold profiles do not add `@webgogol/forge` as a dependency. The `nextSteps` suggesting `forge doctor` are broken.
...
- **`forge doctor` is broken inside new projects** — scaffold profiles (`packages/forge/profiles/*.yaml`) do not add `@webgogol/forge` as a dependency. `pnpm exec forge doctor` inside a newly created project fails with "forge not found". The `nextSteps` suggesting it are incorrect.
...
All scaffold profiles (`packages/forge/profiles/*.yaml`) add `@webgogol/forge` as a devDependency in their install steps. This ensures `forge` binary is available inside the new project via `pnpm exec forge <command>`.

For `forge-shell` profile (minimal): add `pnpm add -D @webgogol/forge` to install steps. For `astro-typescript-turborepo` profile: add `@webgogol/forge` to the root devDependencies install step. For `phaser-turborepo` profile: add `@webgogol/forge` to the root devDependencies install step.
...
6. **Update scaffold profiles** — add `@webgogol/forge` to install steps in all three profiles.
...
- **`@webgogol/forge` version drift** — adding forge as devDependency means the project has its own forge version, which may drift from the global one. Mitigation: `forge.doctor` checks `forge.syncedVersion` and warns on mismatch (already implemented per RFC-0543).
...
- [x] All scaffold profiles (`forge-shell.yaml`, `astro-typescript-turborepo.yaml`, `phaser-turborepo.yaml`) include `@webgogol/forge` in install steps (evidence: packages/forge/profiles/forge-shell.yaml:18, packages/forge/profiles/astro-typescript-turborepo.yaml:57, packages/forge/profiles/phaser-turborepo.yaml:57, stack-profile.test.ts:161-166)
