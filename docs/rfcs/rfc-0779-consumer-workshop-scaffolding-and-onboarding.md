...
### What `workshop.scaffold` generates
...
| `forge.yaml` | Stack set to the chosen profile; bindings referencing `werkstatt run` commands |
| `tools/kernel.config.ts` | Imports engine + selected plugin; registers plugin via `plugins: [werkstattXxxPlugin]` |
| `.npmrc` | Scoped registry config for `@warpgogol` (private npm token placeholder) |
...
| `.agents/` | Forge skills synced via `forge create` |
| `AGENTS.md` | Generated by `forge.agents.generate` with stack-specific instructions |
| `systems/registry.yaml` | Empty registry with schema comment |
| `missions/.gitkeep` | Placeholder |
| `docs/rfcs/rfc-0000-template.md` | RFC template (from forge) |
| `docs/adrs/adr-0000-template.md` | ADR template (from forge) |
| `.forge/pinned.yaml` | Foundation files pinned (DNA-62) |
| `README.md` | Workshop README with quickstart |
...
`workshop.scaffold` runs after generation:
1. `pnpm install` — resolves engine + plugin from npm
2. `forge.doctor` — verifies forge config, skills, bindings
3. `werkstatt.plugin.validate` — verifies plugin registration and profile binding
4. `werkstatt.autonomy.validate` — verifies engine autonomy (no plugin imports in engine)
...
- **Forge `forge.create`** — `workshop.scaffold` delegates to `forge.create` for forge-specific artifacts (skills, AGENTS.md, bindings).
...
- Plugin not installed (npm install fails) → exit 1, `SCAFFOLD-02`.
...
- **`forge.create` as the workshop scaffolder.** Rejected: `forge.create` scaffolds a project inside a workshop; `workshop.scaffold` creates the workshop itself. They are different abstraction levels. `workshop.scaffold` delegates to `forge.create` for forge-specific artifacts.
...
- [ ] SCAFFOLD-01..05 failure modes covered by unit tests
