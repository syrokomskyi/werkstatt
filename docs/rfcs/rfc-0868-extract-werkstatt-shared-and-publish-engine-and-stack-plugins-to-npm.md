...
- [ ] `packages/werkstatt-shared/` exists with `package.json`, `tsconfig.json`, `tsconfig.build.json`, `AGENTS.md` (evidence: `packages/werkstatt-shared/package.json`)
- [ ] `werkstatt-shared/package.json` exports mirror all existing `werkstatt-site` subpath exports for moved domains (evidence: `packages/werkstatt-shared/package.json` exports field)
- [ ] All 6+ shared domains (ontology, share, passport, integration, integration-adapter-supabase-crm, observability, surface, checks) moved to `werkstatt-shared/src/` (evidence: `packages/werkstatt-shared/src/`)
- [ ] No `@warpgogol/werkstatt-site/*` imports remain in `packages/werkstatt/src/**` non-test files (evidence: `werkstatt.shared.validate` exit 0)
- [ ] `EXEMPT_PREFIXES` in `autonomy-validate.ts` contains zero `@warpgogol/werkstatt-site` entries (evidence: `packages/werkstatt/src/plugin/autonomy-validate.ts`)
- [ ] `werkstatt.autonomy.validate` passes with zero violations (evidence: command exit 0)
- [ ] `pnpm build:check` passes for all packages (evidence: CI green)
- [ ] `pnpm test` passes for all packages (evidence: CI green)
- [ ] `@syrokomskyi/axiom-*` moved to `optionalDependencies` in `packages/werkstatt/package.json` (evidence: `packages/werkstatt/package.json`)
- [ ] `leitstand/leitstand-commands.ts` guards axiom import with dynamic import + try/catch (evidence: `packages/werkstatt/src/leitstand/leitstand-commands.ts`)
- [ ] `tsconfig.build.json` added to `werkstatt`, `werkstatt-shared`, `werkstatt-site`, `werkstatt-game`, `werkstatt-godot`, `werkstatt-video` (evidence: `packages/*/tsconfig.build.json`)
- [ ] `exports` updated to dual source/dist in all six packages (evidence: `packages/*/package.json`)
- [ ] `private: false` set on `werkstatt`, `werkstatt-shared`, `werkstatt-site`, `werkstatt-game`, `werkstatt-godot`, `werkstatt-video` (evidence: `packages/*/package.json`)
- [ ] `publishConfig` added to all six packages (evidence: `packages/*/package.json`)
- [ ] `.github/workflows/publish.yml` created with publish-on-tag pipeline (evidence: `.github/workflows/publish.yml`)
- [ ] `workshop.scaffold --name test-godot --stack godot-csharp --dest /tmp/test --verify` passes with `pnpm install` from public NPM (evidence: command exit 0)
- [ ] `werkstatt.shared.validate` command registered in `packages/werkstatt/os/werkstatt-shared-validate.module.ts` and passing (evidence: command exit 0)
- [ ] `werkstatt.shared.validate` output format matches `--json` contract with SHARED-01/02/03 checks (evidence: command output)
- [ ] `AGENTS.md` updated with werkstatt-shared package entry (evidence: `packages/AGENTS.md`)
- [ ] `docs/technology.xml` updated with werkstatt-shared package boundary (evidence: `docs/technology.xml`)
- [ ] `werkstatt-site` published to NPM with `private: false` (evidence: `packages/werkstatt-site/package.json`)
- [ ] `leitstand/leitstand-commands.ts` guards axiom value import with dynamic import, type-only import remains static (evidence: `packages/werkstatt/src/leitstand/leitstand-commands.ts`)
- [ ] `rfc.validate` passes on this RFC before merging (evidence: `rfc.validate --id RFC-0868` exit 0)
