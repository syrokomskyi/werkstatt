...
The `send-message` section gains a generalizable `checklistItems[]` prop that allows sites to define N checklist items, each with a label and a detection rule. The submit button is gated until all items are satisfied. The existing 2-item checklist (length + contact) becomes the default configuration that sites can override, extend, or replace. Detection rules are selected from a closed set of rule types (length, contact-details, url-presence, keyword-match, min-length-custom) to avoid arbitrary code execution from content.

## Architectural fit

- **DNA-17 (Uni manifest contract):** The RFC modifies the `send-message` section's archetype manifest `propsSchema` — replacing individual checklist label props with a `checklistItems[]` array. The manifest remains the authoritative source for the section's prop contract; generated types are regenerated by `props.types.generate`. The manifest version bumps from `1.2.0` to `1.3.0`.
...
- **Forward-only with backward compatibility:** The `checklistItems[]` prop defaults to the current 2-item configuration (length + contact) when absent. Existing sites that don't set `checklistItems[]` continue to work identically. A migrator adds the default `checklistItems[]` to existing sites for explicitness.
...
| `packages/ui/src/sections/send-message/send-message-section.css` | Generalize checklist item styles from `.send-message__checklist-item--length`/`--contact` to `.send-message__checklist-item` with `data-item-id` attribute |
| `packages/os/site-kernel-handoff/src/migrators/rfc-0757.ts` | New migrator (`fromVersion: "4.67.32"`, `toVersion: "4.68.0"`) — adds default `checklistItems[]` to existing sites' send-message blocks for explicitness; no-op on sites without send-message |
...
- **Client-side:** If `checklistItems[]` is absent → fall back to the default 2-item checklist (length + contact). If `checklistItems[]` is present but empty → the checklist is hidden and the submit button is always enabled (no gating). If a rule type is unknown → the item is ignored and a `console.warn` is emitted.
...
- **Empty checklist edge case:** If `checklistItems: []` is explicitly set, the checklist UI is not rendered and the form behaves like a plain textarea + submit (no checklist gating). This is intentional — some forms may not need a checklist.

## Rollout

- **Backward compatibility:** The `checklistItems[]` prop defaults to the current 2-item configuration (length + contact) when absent. Existing sites that don't set `checklistItems[]` continue to work identically — no visual or behavioral change.
- **Migrator (RFC-0479):** A migrator (`rfc-0757.ts`) runs during `mission.migrate` for each site with a `send-message` block. It adds an explicit `checklistItems[]` array with the default 2 items to the block configuration. The migrator is idempotent and a no-op on sites that already have `checklistItems[]` or don't use `send-message`.
...
- **Default migration noise.** The migrator adds explicit `checklistItems[]` to existing sites. If a site has a custom `minMessageLength` value, the migrator must preserve it in the default `checklistItems[]` entry. Mitigation: the migrator reads the existing `minMessageLength` prop and uses it as the `value` for the default length item.
...
- **Visual regression.** Changing the template from 2 hardcoded items to N dynamic items may affect CSS layout. Mitigation: the CSS generalizes from specific item classes to a generic `.send-message__checklist-item` class with `data-item-id` attribute. The 2 default items render identically to the current layout.
...
- [ ] Existing sites with `send-message` blocks continue to work without changes when `checklistItems[]` is absent (evidence: backward compatibility test or manual verification)
...
- Agents MUST NOT remove the individual checklist label props (`checklistTitle`, `checklistReadyLabel`, `checklistLengthLabel`, `checklistContactLabel`) — they serve as backward-compatible fallbacks for sites that don't set `checklistItems[]`.
- Agents MUST NOT weaken or remove enforcement rules established by this RFC without a new RFC that supersedes it.
- If implementation reveals an invariant conflict, run `site-kernel run rfc.supersede.propose --id <this-rfc-id> --reason "..." --invariant "DNA-N"` instead of working around it (RFC-0334).
- The `checklistItems[]` prop is optional. When absent, the section renders the default 2-item checklist (length + contact) using the existing label props. This is the backward compatibility path — do not remove it.
- The migrator must read the existing `minMessageLength` prop value and use it as the `value` for the default length checklist item, preserving site-specific configuration.
