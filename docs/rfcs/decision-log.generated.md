<!--
  ...
  Owner command: rfc.decision-log.generate
  Edit instead: the rfc.decision-log.generate generator source (not this file).
  Regenerate:   forge rfc.decision-log.generate
-->

# RFC Decision Log

> Generated projection of all rejected/superseded RFCs and rejected alternatives.
...
- **Status:** rejected
- **Closed:** 2026-06-08
- **Decision:** The Site OS gains a workspace-scoped command named `cloudflare.regional-services.validate` in `@gogol/site-kernel-checks`.
...
### RFC-0314: Standardize security.txt and vulnerability disclosure for every site

- **Status:** implemented
- **Alternatives considered:**

- **Keep vulnerability contact only on the website.** Rejected; `security.txt` is the standard discovery path.
- **Set a very long expiry.** Rejected; stale security contacts are worse than explicit renewal.
- **Require signed security.txt in v1.** Deferred; the immediate gap is the missing contact file.
...
### RFC-0780: Workshop v5.0.0 major version bump
