...
```ts
// Extended route fact in behavior snapshot
interface RouteFact {
  path: string;
  contentHash: string | null;      // null for redirect routes
...
function isRedirectPage(html: string): boolean {
  // Detects <meta http-equiv="refresh" content="0;url=..."> and <title>Redirecting to:...</title>
}

function extractRedirectTarget(html: string): string | null {
  // Parses the url= value from meta-refresh
}

// Health check: redirect route verification
...
| Path | Role |
| --- | --- |
| `packages/os/site-kernel-handoff/src/leitstand/adapters/cloudflare-workers.ts` | Health check: skip content-hash for `contentHash: null`, verify redirect status |
| `packages/os/site-kernel-handoff/src/behavior-snapshot/behavior-snapshot-capture.ts` | Detect redirect pages, set `contentHash: null` + `redirectTarget` |
| `packages/os/site-kernel-checks/src/behavior-snapshot-generate.ts` | Same redirect detection for `behavior.snapshot.generate` |
| `packages/ontology/src/operations/leitstand.ts` | `RouteFact` type: `contentHash` becomes `string | null`, add optional `redirectTarget` |
...
- **Redirect page detected**: `contentHash` is set to `null`, `redirectTarget` is extracted from the meta-refresh tag. Health check verifies HTTP 307/308 + `Location` header matches `redirectTarget`.
...
- **Existing snapshots**: existing behavior snapshots with redirect routes will have non-null `contentHash` for redirect pages. These will continue to work — the health check will detect a mismatch and report unhealthy. The operator regenerates the snapshot via `behavior.snapshot.generate` to get the new `contentHash: null` format.
- **Backward compatibility**: the health check handles both old format (non-null `contentHash` for redirect routes) and new format (`contentHash: null` + `redirectTarget`). Old format routes are compared as before — the fix only activates for `contentHash: null`.
...
- **Schema change**: `contentHash` changes from `string` to `string | null` in `RouteFact`. This is a backward-compatible schema change — existing consumers that expect `string` will get `null` for redirect routes. Consumers must handle `null` by skipping content comparison.
...
- [ ] `behavior.snapshot.generate` applies the same redirect detection
- [ ] `RouteFact` schema in `@warpgogol/ontology` updated: `contentHash` is `string | null`, optional `redirectTarget`
...
- [ ] Unit tests cover: redirect page detection, redirect health check (307 pass, 200 fail), non-redirect route unaffected
