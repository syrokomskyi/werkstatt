...
export interface DHTPlacementResult {
  ...
  reason: string;            // e.g., "least-loaded", "nearest", "owner-preference"
...
| Path | Role |
| --- | --- |
| `packages/os/site-kernel/src/dht/` | New directory. `node.ts`, `lookup.ts`, `register.ts`, `placement.ts`, `types.ts` modules. |
| `werkstatt.dht.json` | DHT configuration file. Bind address, bootstrap nodes, replication factor, timeouts. Created by `werkstatt.network.bootstrap` (RFC-0562). |
...
| Condition | Behavior |
| --- | --- |
| Site not in DHT | `dht.lookup` returns `found: false`. Workshop may need to register the site or contact the owner. |
| DHT node unreachable (dead workshop) | S/Kademlia routes around dead nodes using disjoint paths. Lookup succeeds via alternative paths. |
| All DHT nodes for a key are dead | `dht.lookup` fails with `key-unavailable` error. The site entry is temporarily unavailable until nodes recover. |
| DHT entry signature invalid | `dht.lookup` returns the entry but marks it as `signature-invalid`. The operator decides whether to trust it. |
| Network partition | Each partition maintains its own DHT view. Entries registered in one partition are not visible in the other. On reunion, DHT converges. |
| Bootstrap node unavailable | `dht.register` fails with `bootstrap-unreachable` error. Operator must provide a reachable bootstrap node. |

## Rollout

- **Phase 1 (single workshop):** No DHT active. The local registry is the only source of site metadata. `dht.lookup` returns `found: false` for all sites. `dht.register` is a no-op (only one workshop).
...
- **Phase 3 (caching):** Workshops cache DHT lookup results locally. Cache TTL is configurable (default 5 minutes). Cached entries are used for subsequent lookups, reducing DHT traffic.
- **Phase 4 (scale):** Hundreds of workshops. S/Kademlia's disjoint paths and Sybil resistance are tested. Replication factor is increased to 5–10 for fault tolerance. Content placement uses capacity metrics from SWIM.
...
- [ ] `DHTSiteEntry`, `DHTConfig`, `DHTLookupResult`, `DHTPlacementResult`, `WorkshopCapacity` types defined in `packages/os/site-kernel/src/dht/types.ts`
...
- [ ] DHT entries are signed with Ed25519 by the registering workshop
- [ ] DHT entries include `owner` field from RFC-0561
- [ ] `dht.lookup` routes around dead workshops (detected by SWIM)
...
- DHT entries MUST be signed with Ed25519 by the registering workshop. Unsigned entries are rejected by `dht.lookup`.
