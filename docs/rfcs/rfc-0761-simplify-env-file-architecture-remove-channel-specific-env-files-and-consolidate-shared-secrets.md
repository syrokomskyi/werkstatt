...
- [ ] `deploy.scripts.validate` checks `deploy:main` and `deploy:alt` use `--secrets-file .env`
- [ ] `env.main.check` and `env.alt.check` commands removed from command table and handlers
- [ ] `env.example.generate` does not include `CLOUDFLARE_API_TOKEN` or `CLOUDFLARE_ACCOUNT_ID` in site template
- [ ] Root `.env.example` includes `WARPGOGOL_OTLP_ENDPOINT` and `WARPGOGOL_OTLP_TOKEN` with `# How to obtain:` instructions
- [ ] `cf-analytics-poller/.env.example` and `fleet-probe-runner/.env.example` do not list `WARPGOGOL_OTLP_*`
- [ ] `package.template.json` in `site-kernel-onboarding` uses `--secrets-file .env` for `deploy:main` and `deploy:alt`
