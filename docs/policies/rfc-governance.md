...
```sh
pnpm exec site-kernel run rfc.list --status accepted --json
```
...
```sh
pnpm exec site-kernel run rfc.create --title "Short imperative title" --kind architecture
```
...
```sh
pnpm exec site-kernel run rfc.check --site <app-name>
```
...
```sh
pnpm exec site-kernel run rfc.acceptance.run --id <rfc-id>
```
