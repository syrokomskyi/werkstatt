...
- **AS-3 — privacy boundary.** `BUSINESS_DOMAIN_VISIBILITY` (`@gogol/share/semantic`) applies verbatim to every agent output; `none`/`pageMeta` domains never reach `public/api/agent/v1/**` — `agent.knowledge.validate` (`AGK-01`) enforces this with the same no-leak technique.
...
- **The MCP endpoint speaks a pinned protocol** (`PINNED_MCP_PROTOCOL_VERSION` in `@gogol/agent-gate`). `agent.gate.fixtures.run` (in `PACKAGES_CHECK_PIPELINE`) is the regression gate for any change to the gate's protocol handling — never touch `packages/agent-gate/src/mcp/**` without keeping its fixture suite green.
...
Agents have read-only access to the fleet telemetry backend (SigNoz) through the `signoz` MCP server configured in `.mcp.json` (env-referenced token `WGOGOL_SIGNOZ_MCP_TOKEN`, viewer role only).

- **MAY** query metrics, traces, logs, alert states, and service views; correlate across `wgogol_probe_*`, `wgogol_factory_*`, `wgogol_delivery_*`, and worker traces by `site_id`.
