...
**Handler path inconsistent with conventions.** The file system responsibilities table (line 159) places the handler at `packages/forge/os/core/create.ts`. Existing handlers follow a different pattern: `runInit` lives in `packages/forge/src/onboarding/init.ts`, `runScaffoldProject` in `packages/forge/src/onboarding/scaffold-project.ts`. The `os/core/core.module.ts` file only _registers_ commands and delegates to `src/` handlers. The handler should be at `packages/forge/src/onboarding/create.ts` with registration in `os/core/core.module.ts`.
