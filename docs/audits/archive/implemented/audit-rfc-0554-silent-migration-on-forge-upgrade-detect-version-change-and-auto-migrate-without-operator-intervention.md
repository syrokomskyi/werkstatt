...
- **Performance** — the RFC says version check is "a single file read and string comparison — negligible overhead." But if the check triggers on every skill invocation, it also involves reading `node_modules/@webgogol/forge/package.json` (I/O) and potentially running migration adapters. The performance claim is only true for the no-op case.
