...
- **Duplicate `isHtmlRedirectPage` not mentioned**: There is a *different* `isHtmlRedirectPage` in `packages/os/site-kernel-checks/src/audit/validators/helpers.ts:73` that checks both `<meta http-equiv="refresh">` AND `window.location.replace|href`, while the share version deliberately excludes `window.location` (per its JSDoc comment). The RFC doesn't mention this duplicate. Moving the share version to `@warpgogol/share/redirects` creates a third location for redirect-detection logic, increasing confusion about which is canonical.
