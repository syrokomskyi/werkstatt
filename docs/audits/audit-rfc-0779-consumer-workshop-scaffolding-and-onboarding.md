...
- **CLI имя `werkstatt run` vs `site-kernel run`.** RFC использует `pnpm exec werkstatt run workshop.scaffold` (line 90), но текущий CLI — `pnpm exec site-kernel run`. Переименование происходит в RFC-0772 (wave 2). RFC-0779 (wave 5) корректно предполагает новое имя, но должно явно отметить зависимость от переименования.
