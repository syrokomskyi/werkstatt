...
- Run `pnpm exec werkstatt run rfc.validate --id RFC-0617`
...
- Stamp the RFC as implemented: run `pnpm exec werkstatt run rfc.implement.stamp --id RFC-0617 --implementation-commit <sha>`

**Validation:**

- `git status` — no uncommitted changes from the current session
- `pnpm exec werkstatt run rfc.validate --id RFC-0617`
...
- If implementation reveals an invariant conflict with DNA-43, run `pnpm exec werkstatt run rfc.supersede.propose --id RFC-0617 --reason "..." --invariant "DNA-43"` instead of working around it.
