...
- Each DRIFT-01 Diagnostic carries: `ruleId: "DRIFT-01"`, `severity: "error"`, `message`, `file` (workspace-relative POSIX path), `data: { generator: string }`, `fixHint: "Re-run: pnpm exec werkstatt run <generator> --site <id>"`
...
- `pnpm --filter @warpgogol/site-kernel-checks run build` — compiles without errors
- `pnpm --filter @warpgogol/site-kernel-checks run test` — existing tests still pass
- Manual verification: run `pnpm exec werkstatt run humans.generate --site warpgogol-com --dry-run --json` and verify `data.renderedFiles` contains the rendered content
...
- Run `pnpm exec werkstatt run rfc.validate --id RFC-0601` — must pass
...
- Run `pnpm exec werkstatt run diagnostic.shape.lint --json` — no DSL-02 warnings for unregistered DRIFT-01/DRIFT-02
...
- **Stamp the RFC as implemented:** run `pnpm exec werkstatt run rfc.implement.stamp --id RFC-0601 --implementation-commit <sha>` to atomically transition `accepted → implemented`.

**Validation:**

- `git status` — no uncommitted changes from the current session
- `pnpm exec werkstatt run rfc.validate --id RFC-0601`
