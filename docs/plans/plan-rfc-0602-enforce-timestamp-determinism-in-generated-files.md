...
- [ ] O1 — Implement `generated.timestamp.validate` command (Phase 1 source lint) — maps to acceptance criteria 1-6
- [ ] O2 — Implement `--deep` flag (Phase 2 standalone drift detection) — maps to acceptance criterion 7
- [ ] O3 — Implement `--mode warning|fail` flag for migration window — maps to acceptance criterion 8
- [ ] O4 — Wire command into `build.check` pipeline (warning mode initially) — maps to acceptance criterion 9
- [ ] O5 — Standard `CheckResult` output with `diagnostics[]` (allowlist exemptions as `info` severity) — maps to acceptance criteria 10-11
- [ ] O6 — Unit tests covering source lint, allowlist, comment/string exclusion, clean pass — maps to acceptance criterion 12
...
- `packages/os/site-kernel-checks/src/generator-ownership.ts` — read-only dependency (`GENERATOR_OWNERSHIP_MAP` provides `module` field for scan targets)
...
### Step 1. Implement `generated-timestamp-validate.ts` module

**Goal:** Create the new command handler with Phase 1 source lint, allowlist, comment/string exclusion, and `--mode`/`--deep` flags.
...
- Implement `runGeneratedTimestampValidate(input, context)`:
  - Extract `--deep`, `--mode` flags from `input.flags` (default `mode: "fail"`)
  - Phase 1 (default): collect unique `module` paths from `GENERATOR_OWNERSHIP_MAP` entries where `module` is defined; read each file; scan line-by-line with comment/string-literal exclusion; match volatile patterns; filter out allowlisted entries; emit `error`-severity diagnostics for violations and `info`-severity diagnostics for allowlist exemptions
  - Phase 2 (`--deep` only): run `build.prepare` twice via `executeKernelPipeline`, diff generated files, emit `error`-severity diagnostics for drifted files with `field` annotation
  - In `warning` mode: downgrade `error` diagnostics to `warning` severity (exit 0)
...
### Step 2. Register command in `01-codegen.ts`
...
- Add entry to `CODEGEN_COMMANDS`:
  ```ts
  {
    ...
    flags: {
      ...
      mode: { type: "string", description: "warning (exit 0) or fail (exit 1)", default: "fail" },
    },
    reads: [
...
### Step 3. Wire into `build.check` pipeline

**Goal:** Add `generated.timestamp.validate` to `SITES_BUILD_CHECK_PIPELINE` in warning mode for the migration window.
...
- The command's default `--mode` is `fail`, but during the migration window the pipeline step should pass `--mode warning`. Since `KernelPipelineStep` does not support per-step flags, the command handler should check `input.flags.mode` — if not specified, default to `fail`. The pipeline step will run with defaults (fail mode) once the migration window closes.
- **Migration window approach:** During the initial rollout, the command handler defaults to `warning` mode when run without explicit `--mode` flag. After all generators are fixed (separate follow-up task), change the default to `fail`. This avoids needing pipeline-level flag passing.
...
**Goal:** Cover source lint detection, allowlist exemption, comment/string exclusion, and clean-pass scenarios.
...
| --------------- | ---------------------- |
| False positives for legitimate timestamps | Step 1: allowlist with `reason` field; Step 4: test allowlist exemption |
| Phase 2 performance | Step 1: `--deep` is opt-in, never in `build.check` pipeline (Step 3) |
| Maintenance burden | Step 1: allowlist is a typed data structure with `module`, `reason`, `pattern` fields |
| Regex false positives | Step 1: comment/string-literal exclusion; Step 4: tests for exclusion cases |
| Cross-package generators | Step 1: reads `GENERATOR_OWNERSHIP_MAP.module` which spans `site-kernel-checks`, `site-kernel-codegen`, `site-kernel-handoff` |
...
- If the `GENERATOR_OWNERSHIP_MAP` `module` field is missing for a critical generator, add the `module` field to the ownership entry first (data fix, not an RFC-level change).
