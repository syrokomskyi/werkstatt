...
The diff fixes all four identified candidates (stub values, exported derive functions, unified input collector, passport projection helper) and adds 24 passing tests. However, two findings require revision before merge: an unused parameter in `collect.ts` that contradicts its own JSDoc, and a stale `AGENTS.md` entry-point table that does not document the new `@gogol/nebula/collect` export.

### Mechanical floor

Pass — `pnpm --filter @gogol/nebula build:check`, `pnpm --filter @gogol/passport build:check`, `pnpm --filter @gogol/site-kernel-checks build:check` all exit 0. `pnpm --filter @gogol/nebula test` — 24/24 passed.

### Axis A — Structural correctness

- **FAIL — unused `label` parameter in `readJsonSafe`.** `@gogol/nebula/src/collect.ts:97` declares `label: string` but never uses it. The JSDoc at line 59 says "Malformed files are logged and ignored" but the `catch` block at line 103 is silent — no `console.warn`, no structured log. Either remove the `label` parameter or use it in a diagnostic log. Dead parameters are a structural smell that linters in other packages already flag.
...
- **FAIL — `AGENTS.md` not updated.** `@gogol/nebula/src/index.ts` now exports `collectNebulaInputs`, `toPassportScores`, 4 derive functions, `PassportScores`, `PassportPillarScore`, and `CollectNebulaInputsOptions`. The package `AGENTS.md` entry-point table (lines 7–12) still lists only `computeNebulaScore`, `createStubNebulaInputs`, and types. The new `@gogol/nebula/collect` export path (added to `package.json` exports) is not documented. Per root `AGENTS.md`: "Keep the affected files synchronized with code, architecture, and verification changes."
- **PASS — package boundaries.** `passport/emit.ts` imports from `@gogol/nebula/compute` and `@gogol/nebula/collect` — both are subpath exports of `@gogol/nebula`, already a `workspace:*` dependency. No new cross-package dependency edges.
...
2. **Should `AGENTS.md` be updated in the same change?** The `packages/nebula/AGENTS.md` entry-point table does not list `@gogol/nebula/collect` or the new exported functions (`toPassportScores`, derive functions). Per the root AGENTS.md Compass duties, should this be updated now?
