...
filesReviewed:
  ...
  - systems/webgogol-com/src/content.config.ts
  ...
  - systems/webgogol-com/AGENTS.md
  - systems/webgogol-com/src/content/AGENTS.md
  - systems/webgogol-com/package.json
---
...
The migration deletes `@gogol/business` and switches to `@gogol/pbp` across the monorepo. The overall direction is correct and forward-only. However, there is one **critical runtime bug** in `people.ts` that will silently break the People section, one **dead-code module** that still execs `pnpm --filter @gogol/business`, and several **RFC deviations** that need reconciliation.
...
- **DNA-20 (business layer)** — PASS. Marked as superseded in `docs/architecture-dna.md`. `@gogol/pbp` is now canonical.
...
- **PASS — AGENTS.md updates.** Root `AGENTS.md`, `packages/AGENTS.md`, `packages/pbp/AGENTS.md`, codegen templates, and webgogol-com AGENTS files all updated to reflect PBP-only.
...
- **FAIL — `cutover-check.ts` still execs `@gogol/business`.** `packages/pbp/src/cutover-check.ts:109` runs `execSync("pnpm --filter @gogol/business build:check")` — this command will fail since the package is deleted. While the OS-side command wrapper was removed (so the command is no longer callable from the pipeline), the module is still exported from `@gogol/pbp/cutover-check` and documented in `packages/pbp/AGENTS.md` line 130. This is dead code that references a deleted package.
...
- **PASS — AGENTS.md clarity.** The critical rule in `packages/pbp/AGENTS.md` clearly states `@gogol/business` is deleted and `@gogol/pbp` is canonical.
...
| `content.config.ts` uses `pbpCollections` | Done | `systems/webgogol-com/src/content.config.ts` |
| Page imports from `@gogol/pbp/semantic-profile` | Done | 4 `.astro` files updated |
...
| `@gogol/business` removed from all package.json | Done | All 7 package.json files updated |
...
| `pnpm --filter @gogol/pbp run test` passes | Done | 169 tests pass |
| `pnpm --filter @gogol/pbp run build:check` passes | Done | tsc --noEmit passes |
...
2. **`cutover-check.ts`**: The module at `packages/pbp/src/cutover-check.ts` still runs `execSync("pnpm --filter @gogol/business build:check")` and is still exported from `@gogol/pbp/cutover-check`. Should this module be deleted or updated to remove the legacy exec?
