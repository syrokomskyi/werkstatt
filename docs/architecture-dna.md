...
A release is an immutable artifact produced from a validated mission. It has a six-digit id (`<system-id>-r<NNNNNN>`), RFC-0269 behavior snapshots (readable and production), a snapshot diff, advisory quality report, release manifest, a durable RFC-0363 artifact reference for the production `dist`, and a public `build-identity.json` written into `dist/client/.well-known/` (RFC-0608). The release artifact state is strictly `prepared | ready` (RFC-0851); deployment progress is a separate append-only `DeploymentOperationState` event chain, not a release manifest field. A release cannot be marked ready unless: (1) the behavior snapshot diff passes (structural parity between readable and production builds), (2) migrator validation passes, (3) the version-compare verdict is not refuse-downgrade, (4) the Bordbuch is consistent, and (5) the release artifact is stored and hash-verified. Legacy deployment-state labels (`published`, `dev-deployed`, `alt-deployed`, `promoted`, `main-deployed`, `rolled-back`) are rejected without translation. Enforced by `release.prepare`, `release.ready`, and `release.validate`. Established by RFC-0357, updated by RFC-0608, RFC-0627, RFC-0628, and RFC-0851.

## DNA-49 · Fleet propagation (Leitstand)

The Leitstand is the fleet operation component that deploys releases to Sternsystem deployment targets via adapter plugins (cloudflare-workers, null). Per-site targeting, rejection of `--all`, explicit channel/URL logging, and Dev → Alt → Main ordering remain mandatory. All site deployment commands (`leitstand.dev-deploy`, `leitstand.propagate`, `leitstand.promote`, `leitstand.status`, `leitstand.rollback`, `leitstand.health`, `leitstand.pipeline.check`) and `release.rollback` are connected to the certification authority via `authorizeDeployment()`, `verifyMainPromotion()`, and `evaluateRollback()` (RFC-0865). Each command requires a `--gate-decision` flag pointing to a `GateDecisionV1` JSON file; `leitstand.promote` additionally requires `--main-verification-decision`. Deployment effect records are written to `systems-cache/{id}/deployment-operations/`. The adapters, locks, build-identity verification, freshness checks, Bordbuch events, and health mechanisms are retained infrastructure. Established by RFC-0358, updated by RFC-0608, RFC-0649, RFC-0698, RFC-0700, RFC-0842, RFC-0851, and RFC-0865.
...
## DNA-64 · Engine/profile/component-graph boundary

The Werkstatt engine (`@warpgogol/werkstatt`) is stack-agnostic and MUST NOT import stack implementations. Every workshop has exactly one Forge stack-profile identity; that profile resolves one immutable, versioned graph of independently lifecycle-managed components rather than one monolithic runtime plugin. Stack packages (`@warpgogol/werkstatt-<stack>`) contribute typed component capabilities for path conventions, validators, codegen, content handling, onboarding, deploy adapters, and stack invariants. The engine owns resolution, lifecycle, effects, and the protected Law Kernel while depending only on capability contracts; stack packages never become a second registry or authority. The consumer-workshop model, package ownership, and engine-to-stack dependency inversion established by RFC-0769 are preserved. The exactly-one-plugin composition clause and `werkstatt/plugin@1` authority are superseded by RFC-0855. Enforcement remains the engine autonomy guard and the RFC-0855 program gates. Established by RFC-0769; superseded and carried forward by RFC-0855.
...
Deployment commands (`leitstand.dev-deploy`, `leitstand.propagate`, `leitstand.promote`) MUST reject the `--all` CLI flag — deployment is always per-site, per-release. Each command MUST log its target channel and URL before executing. The pipeline ordering is strictly sequential: Dev → Alt → Main. `leitstand.propagate` hardcodes channel `alt` (no `--channel` flag); `leitstand.promote` hardcodes channel `main`. The `leitstand.pipeline.check` command provides operators with pipeline state inspection. The old `ready/dev-deployed/alt-deployed/main-deployed` release-state labels are deleted (RFC-0851); deployment progress is now an append-only `DeploymentOperationState` event chain, not a release manifest mutation. All site deployment commands are connected to the certification authority via `authorizeDeployment()`, `verifyMainPromotion()`, and `evaluateRollback()` (RFC-0865). Established by RFC-0842, updated by RFC-0851 and RFC-0865.
...
## DNA-76 · Lifecycle fiber and effect runtime ownership

`packages/werkstatt/src/component-runtime/` owns the structured-concurrency runtime: closed lifecycle state machine, component fibers, four effect handlers (revertible, transactional, compensatable, irreversible-emission), and activation transaction. The lifecycle state machine is closed — invalid transitions are rejected with typed errors. Effect handlers enforce their distinct commit/abort/compensate/withhold laws; failed rollback quarantines. Component fibers own child operations/resources, drain boundedly with deadlines, and unwind effects in LIFO order. Activation transactions drain prior sets in reverse dependency order before committing new sets. No force unload, best-effort teardown, or swallowed disposer errors. No resolver, sandbox, certification, or production activation. Established by RFC-0859.

## DNA-77 · Deterministic component resolution and reconciliation

`packages/werkstatt/src/component-runtime/resolver.ts` owns pure deterministic dependency resolution. The same profile inputs always resolve to one canonical component graph and set hash. Resolution validates manifests, checks artifact availability, verifies admitted grants, matches required capabilities by namespace/compatibility/schema identity, rejects zero/multiple providers, detects cycles, and topologically sorts with canonical component-ID tie-breaking. `reconciliation.ts` computes pure desired-state diff plans (stop/drain/unload/load/activate) and drives RFC-0859 activation transactions. Unchanged setHash is a typed no-op. No package/network discovery, fallback providers, cycle tolerance, or plugin adapters. Established by RFC-0860.

## DNA-78 · Runtime reflection and conformance harness

`packages/werkstatt/src/component-runtime/reflection.ts` owns the read-only, policy-filtered live capability catalog. Catalog entries are canonically ordered, exact-set-bound (bound to the active resolved-set hash), caller-filtered, and schema-strict. The catalog omits secrets, raw grants, private state, credentials, prompts, executable bytes, lease tokens, and authority material — enforced by `assertNoForbiddenFields` and negative tests. `conformance.ts` owns scenario/result contracts where results are always `testOnly: true` and contain no admission/promotion decision. `testing/harness.ts` owns the test-only conformance harness with guards that reject non-test mode, untrusted fixtures, unpinned artifacts, and hash mismatches before import. No production define/install/run/activate/promote command or authority decision is exported. Reflection and conformance results are projections/evidence, not authority. Established by RFC-0861.

## DNA-79 · Provider-neutral isolation contract

`packages/werkstatt/src/isolation/` owns the provider-neutral sandbox adapter contract, deny-by-default capability bridge, attenuated grants, and adversarial conformance suite. `contracts.ts` defines `IsolationAdapterV1`, `SandboxedWorkloadV1`, `CapabilityBridgeRequestV1`/`ResponseV1`, `AttenuatedGrantSetV1`, `WorkloadLimitsV1`, and `IsolationPropertyEvidenceV1` covering twelve required security properties (containment, clean-room startup, artifact immutability, grant enforcement, egress controls, resource limits, secret non-inheritance, teardown, crash recovery, concurrent workload separation, bridge confusion/replay, host compromise assumptions). `schemas.ts` enforces strict Zod validation — unknown fields, invalid grants, replay, confused identity, and all bound violations are rejected. `conformance.ts` runs adversarial cases for filesystem/network/process/env/credential/descriptor escape, resource exhaustion, workload separation, teardown, crash, and bridge confusion. `node:vm`, `worker_threads`, and ordinary subprocesses fail the security-tier contract by definition. Missing property evidence returns `incomplete`, never `pass`. No concrete provider, production activation, credential, network endpoint, or untrusted executable fixture is added. Established by RFC-0862.

## DNA-80 · Certification contracts and identity builders

`packages/werkstatt/src/certification/` owns strict Zod schemas, inferred types, and explicit identity builders for all CERT-001 certification objects. `contracts/` defines identifiers, release candidates, policy bundles, evidence envelopes, dossier events, gate/main/health decisions, action packs, authority artifacts, and deployment operation state/events — all with `.strict()` validation that rejects unknown fields without coercion. `identity.ts` constructs fresh payload objects field-by-field for each identity, snapshots through RFC-0849 `snapshotCanonicalJsonObjectV1`, and hashes through `canonicalJsonHashV1`. Identity payloads include only semantic fields — excluded fields (candidate IDs, evidence IDs, event IDs, timestamps, locators, observed environments) do not affect identity digests. Evidence identity enforces redaction closure: unresolved redaction reports fail with `CERT-REDACTION-01` before identity construction. No evaluation algorithms, state cutover, storage, commands, producers, adapters, authority execution, or deployment logic exists. Established by RFC-0853.

## DNA-81 · Deterministic certification evaluation and remediation

`packages/werkstatt/src/certification/` owns the pure deterministic evaluation pipeline for CERT-001 certification decisions. `evidence-selection.ts` builds a single-pass bounded evidence index in O(E) time, keyed by requirement ID, excluding records above the immutable `evaluationCutSequence` and records without authority admission. Selection chooses the eligible record with greatest authority `admissionSequence` — producer timestamps, filenames, mtimes, lexical IDs, and input array order never establish precedence. `aggregation.ts` resolves required/conditional/advisory requirements, preserves per-requirement `pass | fail | stale | incomplete | not-applicable`, applies top-level precedence `fail > stale > incomplete > pass`, records selected evidence IDs, coverage counts, and reason codes, and never infers pass from zero requirements, zero diagnostics, process exit 0, old evidence, producer crash, timeout, infrastructure unavailability, or unknown values. `action-pack.ts` creates one canonical task per actionable non-pass requirement, validates anchors and verification commands, detects dependency cycles, and returns stable topological order. `dossier-hash.ts` computes order-sensitive event/root hashes. Hard limits: 1,000 requirements, 10,000 evidence records, 1,000 action tasks — all overflow returns explicit non-pass failures without truncation. No function reads/writes `releases/**`, `missions/**`, `systems-cache/**`, object storage, URLs, clocks, environment variables, or provider APIs. Established by RFC-0850.
...
## DNA-83 · Pre-deploy header compatibility validators

Every site build MUST run `csp.elements.validate` and `headers.coverage.validate` in `SITES_CHECK_POSTBUILD_PIPELINE` after `csp.origins.validate` and before `dist.generated-marker.validate`. `csp.elements.validate` cross-references CSP directives (`object-src`, `frame-src`, `media-src`) against HTML elements (`<object>`, `<embed>`, `<iframe>`, `<audio>`, `<video>`, `<source>`) in rendered HTML, falling back to `default-src` when a specific directive is absent. `headers.coverage.validate` cross-references `_headers` path patterns against files in `dist/client/`, emitting warnings for orphan patterns (HDR-COV-01) and errors for tracked file types (`.pdf`, `.mp4`, `.webm`, `.svg`) without matching patterns (HDR-COV-02). Enforced by `csp.elements.validate` (CSP-EL-01..03) and `headers.coverage.validate` (HDR-COV-01..02). Established by RFC-0904.
...
## DNA-87 · Mission close reconcile-freshness and cache-clone gitignore preservation

`mission.close` MUST verify that the workpiece HEAD matches the SHA recorded in `evidence/reconciliation-report.json` before transitioning to closed. Any commits made after the last `mission.reconcile` are unreconciled and MUST block close — the operator must run `mission.reconcile` again to transfer them to the cache clone. `mission.reconcile` MUST preserve cache-clone-only `.gitignore` patterns (forbidden and generated file exclusions) across git merges — the workpiece `.gitignore` is a generated artifact that legitimately differs from the cache-clone `.gitignore`, and the merge must not overwrite the cache-clone additions. After merge, `mission.reconcile` MUST untrack any forbidden/generated files that were re-tracked by the merge. Without these guards, workpiece commits are silently lost and `sternsystem.validate` reports `bundle-contract` violations after every close. Enforced by `mission.close` (reconcile-freshness gate) and `mission.reconcile` (`.gitignore` restoration + `git rm --cached` cleanup). Established by RFC-0913.

## DNA-88 · Canonical slug generation ownership

All slug generation in the monorepo uses `@warpgogol/werkstatt-shared/share/slug`. The external packages `@sindresorhus/slugify`, `cyrillic-to-translit-js`, and `github-slugger` are dependencies of `werkstatt-shared` only — no other package may import them directly. Ad hoc `slugify()` reimplementations are forbidden outside the canonical module. The canonical module exports `slugUrl` (locale-aware URL slug), `slugId` (semantic block ID), and `HeadingSlugger` (heading anchor deduplication). Enforced by `utility.provenance.validate` (RFC-0916). Established by RFC-0915.
