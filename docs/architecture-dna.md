...
The Leitstand is the fleet operation component that deploys releases to Sternsystem deployment targets via adapter plugins (cloudflare-workers, null). Per-site targeting, rejection of `--all`, explicit channel/URL logging, and Dev → Alt → Main ordering remain mandatory. However, all site deployment commands (`leitstand.dev-deploy`, `leitstand.propagate`, `leitstand.promote`, `leitstand.status`, `leitstand.rollback`, `leitstand.health`, `leitstand.pipeline.check`) are currently blocked with `CERT-TRANSITION-01` until CERT-007 reconnects them to signed certification authorization. The adapters, locks, build-identity verification, freshness checks, Bordbuch events, and health mechanisms are retained infrastructure but unreachable from blocked site commands. `release.rollback` is likewise blocked. Established by RFC-0358, updated by RFC-0608, RFC-0649, RFC-0698, RFC-0700, RFC-0842, and RFC-0851.
