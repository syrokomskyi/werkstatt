...
- The plan describes steps but does NOT authorize execution when invoked directly by the user or via a plan-only orchestrator (`fo-idea-i-just-want-to-see-the-plan`, `stopAfter: plan`) — use `/fo-idea-implement` to execute. When invoked as part of a full-pipeline orchestrator without `stopAfter: plan` (e.g. `fo-idea-i-just-want-to-see-the-result` with default `stopAfter: null`), the orchestrator's instruction to continue to the next pipeline step takes priority — do not stop after the plan is committed; proceed to implementation as directed by the orchestrator.
