...
- The plan describes steps but does NOT authorize execution when invoked directly by the user — use `/fo-idea-implement` to execute. When invoked as part of a pipeline orchestrator (e.g. `fo-idea-i-just-want-to-see-the-result`), the orchestrator's instruction to continue to the next pipeline step takes priority — do not stop after the plan is committed; proceed to implementation as directed by the orchestrator.
