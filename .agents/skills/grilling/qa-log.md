...
### K-0039: One universal prompt vs per-profile prompts for README no-terminal start
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the README provide one universal prompt or three separate prompts (one per stack profile)?
- **Answer:** One universal prompt. The agent maps the operator's project description to the correct profile. Three blocks force the operator to know their profile before copying — defeats the purpose of a no-terminal path for non-programmers.

### K-0040: Greenfield-only prompt vs covering transplant in README no-terminal start
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the no-terminal prompt cover both greenfield and transplant scenarios?
- **Answer:** Greenfield only. Transplant requires a source project path — an advanced scenario the beginner prompt shouldn't carry. /forge-bootstrap step 5 already asks "greenfield or transplant?" and handles the alternative path naturally.

### K-0041: Operator fills placeholder before copying vs agent asks after
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the operator fill the [describe your project] placeholder before copying the prompt, or should the agent ask what to build after receiving a bare prompt?
- **Answer:** Operator fills before copying. forge create requires --profile before bootstrap asks what to build — the agent can't select a profile without a description. A placeholder in the prompt is one extra step for the operator but eliminates a round-trip.

### K-0042: Mentioning /forge-bootstrap in the README prompt
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the no-terminal prompt explicitly mention /forge-bootstrap?
- **Answer:** No. Skill names with / prefix are internal jargon. The prompt says "install and set up" — the agent discovers NEXT_STEPS.md after forge create and runs bootstrap automatically. Shorter prompt, no jargon.

### K-0043: npm URL in the README prompt
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the prompt include the npm package URL or just the package name?
- **Answer:** Include the URL directly: `https://npmjs.com/package/@warpgogol/forge`. The URL contains the package name and gives the agent an unambiguous source — it can read the npm page for description, version, and README. No duplication of the package name alongside the URL.

### K-0044: Prerequisites in the no-terminal start block
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the no-terminal start block mention Node.js/pnpm prerequisites?
- **Answer:** No. The block's value is minimalism. If prerequisites are missing, the agent sees the error and guides the operator. The "Complete installation guide" section below covers full setup for those who want to prepare.

### K-0045: New block vs replacing existing Quick start section
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the no-terminal start block replace the existing "For creative operators" section or be added alongside it?
- **Answer:** Add alongside, before the existing section. The existing CLI path works and is verified. Two paths — choice, not confusion. The new block is the simplest path; the existing one is for operators comfortable with the terminal.

### K-0046: Prompt format — blockquote vs code block
...
- **Context:** 2026-08-19 — grilling README no-terminal start block for @warpgogol/forge
- **Question:** Should the copy-paste prompt be in a blockquote or a code block?
- **Answer:** Code block. The instruction text is plain markdown; the prompt is a copyable artifact. Code blocks are the conventional format for copy-paste content in README files. Blockquotes render as citations, not copy targets.
