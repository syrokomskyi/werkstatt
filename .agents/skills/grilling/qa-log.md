...
### K-0009: check-core classification — engine or site plugin
...
- **Context:** 2026-08-09 — grilling RFC-0771 enhance (werkstatt engine core package composition)
- **Question:** `check-core` (Zod schemas for check runs, evidence graphs, reports) has no Astro dependencies and depends only on engine packages (`fingerprint`, `site-kernel`). Should it be engine or site plugin?
- **Answer:** Site plugin. RFC-0769 charter does not list check framework as an engine concern. Quality gate execution is a plugin hook (RFC-0770). The check schema framework is generic, but the check ecosystem (Axiom, evidence capture) is site-specific. Type-only dependency on engine packages does not make a package engine.

### K-0010: studio-gate classification — engine or site plugin
...
- **Context:** 2026-08-09 — grilling RFC-0771 enhance (werkstatt engine core package composition)
- **Question:** `studio-gate` (stdio MCP server for site owner content editing, DNA-56) has no Astro dependencies and depends on engine packages (`site-kernel`, `site-kernel-handoff`). Should it be engine or site plugin?
- **Answer:** Site plugin. DNA-56 defines studio-gate as a site-specific MCP server for content editing. It depends on `passport` (site domain). Game/video workshops may have their own MCP servers. Tool projection (workpiece.read/write) is site-oriented even though the underlying commands are engine. Dependency on engine packages does not make a package engine — the engine is for lifecycle/platform concerns, not domain-specific tooling.
