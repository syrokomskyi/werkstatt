...
Read `systems/registry.yaml`. If exactly one active system exists, use it. If multiple, ask the operator to choose. Resolve the site id (e.g. `webgogol-com`) and the current mission id.
...
6. Commit knowledge file updates to the main repo: `git add packages/wgogol-skills/skills/wg-site-scan/learned-principles.md packages/wgogol-skills/skills/wg-site-scan/qa-log.md && git commit -m "chore: update wg-site-scan knowledge from run"`.
