/*
<MODULE_CONTRACT>
<purpose>game.assets.validate — checks asset manifest completeness (GAME-02, RFC-0777).</purpose>
<keywords>validator, assets, game, manifest</keywords>
<non-goals>
  <item>Does not modify files — read-only validator.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>RFC-0777: initial assets validator — reads manifest, checks referenced assets exist.</item>
</CHANGE_SUMMARY>
*/
...
export async function validateAssets(
  ...
  // Check each manifest entry exists on disk
  ...
  // Check for files in src/assets/ not listed in manifest
  ...
  return {
    data: { command: "game.assets.validate", status, violations },
    exitCode: status === "pass" ? 0 : 1,
    summary: `game.assets.validate: ${status} (${violations.length} violations)`,
  };
}
...
export function createAssetsValidateCommand(): KernelCommandDefinition<AssetsValidateData> {
  return {
    name: "game.assets.validate",
    description: "Validate game asset manifest completeness (GAME-02)",
    scope: "workspace",
    cacheable: false,
