/*
<MODULE_CONTRACT>
<purpose>game.secret.scan — GAME-04 secret scan enforcement (RFC-0777).</purpose>
<keywords>validator, secret, scan, security, game</keywords>
<non-goals>
...
<CHANGE_SUMMARY>
  <item>RFC-0777: initial secret scan — regex patterns for API keys, tokens, passwords in src .ts files.</item>
</CHANGE_SUMMARY>
*/
...
export async function scanSecrets(
  ...
  return {
    data: { command: "game.secret.scan", status, violations },
    exitCode: status === "pass" ? 0 : 1,
    summary: `game.secret.scan: ${status} (${violations.length} violations)`,
  };
}
...
export function createSecretScanCommand(): KernelCommandDefinition<SecretScanData> {
  return {
    name: "game.secret.scan",
    description: "Scan game source for hardcoded secrets (GAME-04)",
    scope: "workspace",
    cacheable: false,
