...
## Leitstand (RFC-0358 / RFC-0379 / RFC-0608 / RFC-0627)
...
- `leitstand.deploy` (RFC-0627) deploys a `published` or `dev-deployed` release to the `dev` channel. After successful deployment, it runs the Axiom verification gate via `mission.check --external-preview --base-url=<dev-url>`. On success, the release state transitions to `dev-deployed`. The Axiom gate result (pass/fail/not-run) is included in the command output.
- `leitstand.propagate` deploys a `dev-deployed` release with verified Axiom evidence to the `alt` channel (RFC-0627). The `--channel` flag is removed; passing it throws with a message directing to `leitstand.promote`. Before deployment, it verifies that `missions/{missionId}/evidence/axiom/findings.yaml` exists, has zero errors, and was recorded after the release's `publishedAt`. On success, the release state transitions from `dev-deployed` to `alt-deployed`.
...
- `leitstand.rollback` auto-detects the channel from the current release state (RFC-0627): `promoted` → `main`, `alt-deployed` → `alt`, `dev-deployed` → `dev`. The `--channel` flag is removed; passing it throws. After rollback, the release state auto-steps one step back in the deployment chain: `promoted` → `alt-deployed`, `alt-deployed` → `dev-deployed`, `dev-deployed` → `published`. The state transition is non-fatal if the release manifest is missing.
...
- `leitstand.rollback` auto-detects the channel from the release state (RFC-0627). The `--channel` flag is removed.
