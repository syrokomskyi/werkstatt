...
- `leitstand.dev-deploy` (RFC-0628) deploys the active mission's workpiece to the `dev` channel. It takes `--system` (not `--release`), resolves `currentMission` from the registry, builds the workpiece via `pnpm build`, captures the workpiece HEAD sha, deploys to dev via the adapter, purges CDN cache, runs the Axiom verification gate via `mission.check --external-preview --base-url=<dev-url> --commit-sha=<sha>`. The commit SHA is passed directly to `mission.check` which embeds it in `evidence-metadata.json` — no post-processing of evidence files. It does NOT write to the registry or bordbuch — dev deploys are ephemeral. The Axiom gate result (pass/fail/not-run) is included in the command output.
