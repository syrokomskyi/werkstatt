...
- `leitstand.dev-deploy` (RFC-0628) deploys the active mission's workpiece to the `dev` channel. It takes `--system` (not `--release`), resolves `currentMission` from the registry, builds the workpiece via `pnpm build`, captures the workpiece HEAD sha, deploys to dev via the adapter, purges CDN cache, runs the Axiom verification gate via `mission.check --external-preview --base-url=<dev-url>`, and post-processes `evidence-capsule.yaml` with `commitSha`. It does NOT write to the registry or bordbuch — dev deploys are ephemeral. The Axiom gate result (pass/fail/not-run) is included in the command output.
- `leitstand.propagate` deploys a `published` release with verified Axiom evidence to the `alt` channel (RFC-0628). The `--channel` flag is removed; passing it throws with a message directing to `leitstand.promote`. Before deployment, it verifies that `missions/{missionId}/evidence/axiom/evidence-capsule.yaml` exists with `missionId` and `commitSha` matching the release manifest, and that `findings.yaml` has zero errors. On success, the release state transitions from `published` to `alt-deployed`.
