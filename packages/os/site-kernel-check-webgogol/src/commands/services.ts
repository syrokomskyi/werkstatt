/*
<MODULE_CONTRACT>
<purpose>Maintains packages/os/site-kernel-check-webgogol/src/commands/services.ts as an authored site-kernel-check-webgogol authored module so agents can evolve it without rediscovering local boundaries.</purpose>
<non-goals>
...
export async function runCheckWebgogolRunnerValidate(
  _input: KernelCommandInput,
  context: KernelRuntimeContext,
): Promise<KernelCommandResult<CheckResult>> {
  const diagnostics: Diagnostic[] = [];
  const required = [
    "services/check-webgogol-runner/package.json",
    "services/check-webgogol-runner/service.config.yaml",
    "services/check-webgogol-runner/Dockerfile",
    "services/check-webgogol-runner/src/worker.ts",
  ...
  for (const file of required) {
    if (!(await context.io.exists(join(context.workspaceRoot, file)))) {
      diagnostics.push({
        ...
        severity: "error",
        file,
        message: `Required Check Webgogol runner file is missing: ${file}`,
        fixHint: "Create the required file from RFC-0304.",
      });
    }
  }
  const packagePath = join(context.workspaceRoot, "services/check-webgogol-runner/package.json");
  if (await context.io.exists(packagePath)) {
    const pkg = JSON.parse(await context.io.readFile(packagePath)) as {
      dependencies?: Record<string, string>;
    };
    for (const dep of ["@gogol/check-core", "@gogol/check-runner-node"]) {
      if (!pkg.dependencies?.[dep]) {
        diagnostics.push({
          ruleId: "CW-RUNNER-02",
          severity: "error",
          file: "services/check-webgogol-runner/package.json",
          message: `check-webgogol-runner must depend on ${dep}.`,
          fixHint: `Add ${dep} as a workspace dependency.`,
        });
      }
    }
  }
  const runnerSources = await context.io.glob("services/check-webgogol-runner/src/**/*.ts", {
    cwd: context.workspaceRoot,
  });
  for (const source of runnerSources) {
    const text = await context.io.readFile(join(context.workspaceRoot, source));
    if (text.includes("apps/") || text.includes("@gogol/check-webgogol-com")) {
      diagnostics.push({
        ruleId: "CW-RUNNER-03",
        severity: "error",
        file: source,
  ...
  const workerPath = join(context.workspaceRoot, "services/check-webgogol-runner/src/worker.ts");
  if (await context.io.exists(workerPath)) {
    const worker = await context.io.readFile(workerPath);
    if (!worker.includes("@gogol/check-core") && !worker.includes("./run-once.ts")) {
      diagnostics.push({
        ruleId: "CW-RUNNER-04",
        severity: "error",
        file: "services/check-webgogol-runner/src/worker.ts",
        message: "Runner worker does not use shared Check Webgogol run contracts.",
        fixHint: "Import run contracts from @gogol/check-core directly or through run-once.ts.",
      });
    }
  }
  const apiFiles = [
  ...
  for (const file of apiFiles) {
    ...
    if (
      ...
      text.includes("@gogol/check-runner-node") ||
      ...
      diagnostics.push({
        ...
        fixHint: "Keep browser execution in services/check-webgogol-runner.",
      });
    }
  }
  return diagnosticsResult("check-webgogol.runner.validate", diagnostics);
}
