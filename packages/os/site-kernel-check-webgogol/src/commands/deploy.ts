/*
<MODULE_CONTRACT>
<purpose>Deploy alt run, main gate, artifact validation, and check run command handlers for check-webgogol.</purpose>
<non-goals>
...
import { captureSiteEvidenceGraph } from "@gogol/check-runner-node";
import {
    ...
    return diagnosticsResult("check.deploy-main.gate", [
      {
        ...
        fixHint: "Pass --report <report.json> or --run .check-webgogol/runs/<runId>/run.json.",
      },
    ]);
  }
    ...
    return diagnosticsResult("check.artifact.validate", [
      {
        ...
        fixHint: "Pass --run .check-webgogol/runs/<runId>/run.json.",
      },
    ]);
  }
  try {
    ...
    if (!absolute.endsWith(join(runRelDir(artifact.runId), "run.json"))) {
      diagnostics.push({
        ruleId: "CW-ART-03",
        severity: "warning",
        message: "run.json is outside the canonical .check-webgogol/runs/<runId>/ layout.",
