...
import { discoverSiteWorkspaces, writeFileIfChanged } from "@gogol/site-kernel";
...
export interface RegistryEntry {
  ...
  standalone: boolean;
  /** Kernel app name (e.g. "nicaragua-projekt"), or "@gogol/ui" for shared packages. */
  siteName: string;
 ...
 * parses each against @gogol/ontology manifestSchema, and emits a
    ...
    if (!result.success) {
      diagnostics.push({
        ...
        message: `Manifest fails schema validation: ${result.error.issues
          ...
          .join("; ")}`,
        file: relFile,
        fixHint: "Fix the manifest to satisfy @gogol/ontology manifestSchema.",
      });
      continue;
    }
