...
export const SECTION_FRAMEWORK_COMMANDS: CheckCommandEntry[] = [
  ...
  /* RFC-0124 */
  {
    ...
    description:
      "Verify every --ds-* token referenced under the eight section-framework component directories exists in @gogol/tokens TOKEN_NAME_SET (RFC-0108 + RFC-0124).",
    scope: "workspace",
    flags: {},
    supportsAllSites: true,
    reads: [
  ...
  /* tokens.catalog.sync — drift guard between tokens.css and TOKEN_NAMES */
  {
    ...
    description:
      "Verify that every --ds-* custom property in packages/tokens/src/tokens.css is listed in @gogol/tokens TOKEN_NAMES and vice versa. Reports drift in both directions.",
    scope: "workspace",
    flags: {},
    supportsAllSites: true,
