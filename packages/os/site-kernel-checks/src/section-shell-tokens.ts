...
// exported by @gogol/tokens). Catches drift where a section primitive
  ...
  for (const file of files) {
    ...
    for (const match of cleanText.matchAll(TOKEN_REF_REGEX)) {
      ...
      violations.push({
        ...
        message: `Section-framework CSS references ${token} but the token is not in @gogol/tokens TOKEN_NAME_SET.`,
