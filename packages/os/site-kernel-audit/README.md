...
The handler is registered as a kernel command by `@gogol/site-kernel-checks` (inside `createStandardCheckModule`), which imports `runAuditDeltaRun` from here. The `amend-check.postbuild` composite invokes `audit.delta.run` by name via `executeKernelCommand`, so there is no import coupling to the composite.
