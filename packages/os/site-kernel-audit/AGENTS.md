# AGENTS — @gogol/site-kernel-audit
...
- Dispatch other validators by name via `executeKernelCommand`. Do NOT import validator handlers directly — keep this package's dependency surface minimal (`@gogol/site-kernel` only).
...
- Registration of the command lives in `@gogol/site-kernel-checks`, not here.
