...
test("createCompassInventoryEntries excludes generation templates but keeps authored neighbors", async () => {
  ...
  try {
    ...
    await mkdir(join(root, "packages", "os", "site-kernel", "src", "templates"), {
      recursive: true,
    });
    await writeFile(
    ...
    await writeFile(
      join(root, "packages", "os", "site-kernel", "src", "templates", "kernel.template.ts"),
      "export const template = '{{KERNEL}}';\n",
      "utf8",
    );
    await writeFile(
      join(root, "packages", "os", "site-kernel", "src", "runtime.ts"),
    ...
    const osTemplate = entries.find(
      (entry) => entry.path === "packages/os/site-kernel/src/templates/kernel.template.ts",
    );
    const osAuthored = entries.find(
      (entry) => entry.path === "packages/os/site-kernel/src/runtime.ts",
    );
