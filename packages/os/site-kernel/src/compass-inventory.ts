/*
<MODULE_CONTRACT>
<purpose>Provides Compass inventory scanning logic shared between site-kernel-checks and site-kernel-codegen without creating a circular dependency.</purpose>
<non-goals>
  <item>Do not render XML output — that belongs in site-kernel-checks compass.ts.</item>
  <item>Do not register kernel commands — this is a pure utility module.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
...
function getFlagValues(input: KernelCommandInput, key: string): string[] {
...
function resolveScanRoots(workspaceRoot: string, input: KernelCommandInput): string[] {
...
function detectAuthoringStatus(
  ...
  // RFC-0081: files carrying the canonical GENERATED_MARKER are generator-owned
  // (e.g. the api.routes.generate Astro APIRoute re-exports under src/pages/api/).
  // They are governed by generated.marker.validate, not the authored
  // MODULE_CONTRACT requirement, so the path-based heuristics below are not the
  // only signal — the in-file marker is authoritative for generated status.
