...
- Commands that resolve a site's working tree directory MUST NOT hardcode `join(workspaceRoot, "systems", app)` or `join(workspaceRoot, "apps", app)`. The `systems/` directory only contains `registry.yaml` (RFC-0381 retired `apps/`); actual site content lives in the cache clone at `mirrors[0].path` declared in `systems/registry.yaml`. Use a `resolveAppDir()` helper that tries `systems/<app>` first (backward compat), then falls back to reading `systems/registry.yaml` mirrors[0].path. See `config-template-sync.ts` and `generated-files-validate.ts` for the established pattern. `config.regenerate` still has this bug — fix it when touched.
