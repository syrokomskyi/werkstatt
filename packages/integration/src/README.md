...
| `port-barrel.ts` | Types-only barrel (`@gogol/share/integration/port`) — re-exports port + sibling type modules without pulling in adapter implementations. Type-only consumers (agent-gate, supabase-crm tests) import from here. |
| `index.ts` | Full barrel (`@gogol/share/integration`) — types + runtime (registries, fan-out, delivery handler). |
...
`node --import tsx --test src/integration/tests/*.test.ts` (run via `pnpm --filter @gogol/share test`). Covers QStash publish + Redis ledger, the dispatch seam, sharding, and the inbound/route runtime.
