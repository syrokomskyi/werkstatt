# `@warpgogol/content-source` — Agent Guide
...
| --- | --- | --- |
| `@warpgogol/content-source` | `src/types.ts` | Port contracts: `ContentSourceProvider`, `ContentDomain`, `ContentEntry`, `ContentEntryRef`, `AssetRef`, `ResolvedAsset`, `ContentSourceCapabilities` |
| `@warpgogol/content-source` | `src/adapters/fs/assets.ts` | The relocated bare-filename image resolver (`resolveImage`, `resolveImageRequired`, `createImageResolver`, `IMAGE_EXTENSIONS`, `DEFAULT_LANGUAGE`) **and** `createFsAssetResolver(images)` → the abstract `AssetRef → ResolvedAsset` resolver |
| `@warpgogol/content-source` | `src/adapters/fs/loaders.ts` | `fsMarkdownCollectionLoader`, `fsDataCollectionLoader` |
| `@warpgogol/content-source` | `src/adapters/fs/capabilities.ts` | `FS_CAPABILITIES` |
| `@warpgogol/content-source/cms-git` | `src/adapters/cms-git/index.ts` | **RFC-0171** git-based (Decap) adapter: `CMS_GIT_CAPABILITIES` + the pure, node-safe Decap config builder (`buildDecapConfig`, `inferDecapField`, `mergeSamples`). No I/O, no `yaml`, no `astro:content` — the `cms.schema.generate`/`cms.schema.parity` kernel commands read content and serialize. Git markdown is read through the fs Astro provider, so production resolves only merged (published) content (fail-closed). |
| `@warpgogol/content-source/astro` | `src/astro.ts` | The **only** module that owns the `astro:content` dependency: re-exports `getEntry` / `getCollection` |
...
- **Content reads** import `getEntry` / `getCollection` from `@warpgogol/content-source/astro`, never from `astro:content` directly. `@warpgogol/share`'s `astro/content.ts` + `astro/page-handler.ts` already route through it.
- **Collection loaders** come from the fs adapter. `@warpgogol/share/astro/loaders` re-exports `markdownCollectionLoader` (= `fsMarkdownCollectionLoader`) so generated `content.config.ts` is unchanged; `@warpgogol/pbp` builds its collection via `fsDataCollectionLoader`.
...
- Do NOT re-add a per-component `import.meta.glob` for content assets. Use `contentAssetImages` from `@warpgogol/ui/src/content-assets.ts`. `asset.reference.validate` exists to catch unresolved tokens (warning mode today; will be promoted to fail-hard).
- Do NOT import `astro:content` anywhere new. Import the accessors from `@warpgogol/content-source/astro`.
- Do NOT reimplement bare-filename image resolution. It lives in `src/adapters/fs/assets.ts`; `@warpgogol/share` re-exports it for backward compatibility.
...
- `content.source.parity` (in `@warpgogol/site-kernel-checks`) is the migration guard: the fs adapter's enumeration must equal the on-disk content inventory. Keep it green when changing loaders or id derivation.
