...
| `@warpgogol/share/content-reference` | `src/content-reference.ts` | RFC-0527 content reference index resolver — `ContentRefIndex`, `loadContentRefIndex`, `getContentRefIndex`, `resolveReference`, `resolveReferencesInString`, `resolveReferencesDeep`, `EMPTY_CONTENT_REF_INDEX`, `SourceRef`. RFC-0570: `resolveReferencesInString` now handles `=(...)` formula expressions. RFC-0731: `SourceRef` optional parameter enables `this.` self-reference expansion (`this.field` → `collection.file.field`) |
| `@warpgogol/share/shared-context` | `src/shared-context.ts` | RFC-0099 page-driven shared context fallback helpers |
| `@warpgogol/share/formula-eval` | `src/formula-eval.ts` | RFC-0570 formula evaluation — `extractNumeric`, `scanFormulas`, `resolveFormula`, `FormulaResolution`. RFC-0723: `resolveFormula` returns string values for single-ref expressions (no arithmetic), enabling `=(ref)` for string interpolation in mixed strings. RFC-0729: pipe syntax for post-evaluation formatting, formatter registry (`registerPipeFormatter`, `getPipeFormatter`, `PipeFormatter`, `PipeFormatterContext`), and `money` formatter built on `Intl.NumberFormat`. RFC-0731: `sourceRef` optional parameter enables `this.` self-reference expansion in formula expressions |
