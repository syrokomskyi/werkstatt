...
| `@warpgogol/share/formula-eval` | `src/formula-eval.ts` | RFC-0570 formula evaluation — `extractNumeric`, `scanFormulas`, `resolveFormula`, `FormulaResolution`. RFC-0723: `resolveFormula` returns string values for single-ref expressions (no arithmetic), enabling `=(ref)` for string interpolation in mixed strings |
