 ...
 */
export async function resolveSectionAnchor(props: any, defaultAnchorId?: string): Promise<string> {
  const anchorId = props.pageOverride?.anchorId ?? defaultAnchorId;
  if (!anchorId) return "";
  if (props.pageId) {
    return resolveAnchorFragment(anchorId, props.pageId, props.lang);
  }
