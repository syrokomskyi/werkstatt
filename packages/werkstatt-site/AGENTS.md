...
- `jsonld.canonical-entity.validate` (RFC-0910) — scans rendered HTML in dist/client/ for JSON-LD entity nodes and validates that `Organization.url`, `WebSite.url`, `BreadcrumbList` home item, and same-origin `Person.url` are canonical (unprefixed root, no default-language prefix like `/de/`). Emits JSONLD-ENTITY-01 (Organization/WebSite url not canonical root, error), JSONLD-ENTITY-02 (BreadcrumbList home item carries language prefix, error), JSONLD-ENTITY-03 (same-origin Person.url carries default-language prefix, error). External Person URLs (different origin) are skipped. Integrated into `SITES_CHECK_POSTBUILD_PIPELINE` after `jsonld.url.validate` and before `jsonld.parity`.
...
- `seo.meta-uniqueness.validate` (RFC-0911) — validates that no two indexable pages in the same language share an identical `<title>` (SEO-UNIQ-01, error) or meta description (SEO-UNIQ-02, error). Skips noindex, redirect, and `.well-known` pages. Uniqueness is evaluated per language; translated sister pages never collide. Integrated into `SITES_CHECK_POSTBUILD_PIPELINE` after `seo.meta.validate`.
- `seo.anchor-text.validate` (RFC-0911) — lints rendered internal links for generic anchor text (SEO-ANCHOR-01, error) using a built-in de/uk stop-list extensible via `system.md` `seo.anchorText.extraStopPhrases`. Also warns on bare URL anchor text (SEO-ANCHOR-02, warning). Whole-text match only — descriptive sentences containing a stop word do not trigger. Skips noindex, redirect, and `.well-known` pages. Integrated into `SITES_CHECK_POSTBUILD_PIPELINE` after `seo.meta-uniqueness.validate`.
...
- `video.structured-data.validate` (RFC-0912) — validates that every opted-in content video block (`seo.videoObject: true`) renders a complete VideoObject JSON-LD node, that no non-opted-in or decorative video carries VideoObject markup, and that `sitemap-video.xml` entries match opted-in videos. Emits VIDEO-SEO-01 (opted-in block missing required field: name/description/uploadDate, error), VIDEO-SEO-02 (opted-in video has no VideoObject in rendered HTML, error), VIDEO-SEO-03 (rendered VideoObject traces to non-opted-in or decorative block, error), VIDEO-SEO-04 (sitemap-video.xml entry missing or spurious, error), VIDEO-SEO-05 (VideoObject duration absent, warning). Integrated into `SITES_CHECK_POSTBUILD_PIPELINE` after `seo.structured-data.validate`.
