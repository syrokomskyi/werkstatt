...
- **WCAG 2.5.3 component-level rule (ADR-0047, RFC-0834):** Components that render both `aria-label` and visible text on the same interactive element MUST merge the visible text into the aria-label. The merge pattern is `resolvedAriaLabel = ariaLabel && label && !ariaLabel.includes(label) ? \`${label} — ${ariaLabel}\` : ariaLabel`. Applied to `section-cta.astro`, `hero-section.astro`, `brand-label-component.astro` (ADR-0047). RFC-0834 proposes a component-level validator (`a11y.label-in-name.component.validate`) to catch this pattern at authoring time, before the build runs.
