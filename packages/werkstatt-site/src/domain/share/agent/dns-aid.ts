...
<purpose>
  ...
  at _index._agents.<domain> with alpn and endpoint params pointing to the
  agent.json manifest URL.
</purpose>
<non-goals>
 ...
 * The record is an SVCB service record with:
...
export function buildDnsAidRecord(manifest: AgentSurfaceManifest): DnsAidRecord {
  ...
  const endpoint = `${origin}/.well-known/agent.json`;
  return {
    name: `_index._agents.${domain}`,
    type: "SVCB",
    content: `1 . alpn=h2 endpoint=${endpoint}`,
    ttl: 3600,
    proxied: false,
  };
}
