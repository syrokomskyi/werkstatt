...
<CHANGE_SUMMARY>
  ...
  <item>RFC-0175 fix: invert the adapter dependency — the host injects the loader map so
        @warpgogol/werkstatt-site/chat does not depend on the adapter packages (removes the build:check cycle).</item>
</CHANGE_SUMMARY>
*/
 ...
 * section client in @warpgogol/werkstatt-site/ui) owns this map with STATIC `import()` specifiers so the
 * bundler code-splits each adapter into a resolvable async chunk. Keeping it here (in
 * the host, not the port) avoids @warpgogol/werkstatt-site/chat depending on the adapter packages — which
 * would form a workspace cycle (adapters already depend on @warpgogol/werkstatt-site/chat).
 */
