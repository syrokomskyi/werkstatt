...
<MODULE_CONTRACT>
<purpose>
  Evidence reading and run-artifact management for check-warpgogol OS commands.
...
export async function readEvidenceForRun(
  ...
  if (!run && !evidence) {
    diagnostics.push({
      ...
      fixHint:
        "Pass --run .check-warpgogol/runs/<runId>/run.json or --evidence <evidence.graph.json>.",
    });
