...
behavior from the archetype registry (@warpgogol/werkstatt-shared/ontology/archetypes), never from
...
import { StarCatalog, PlanetCatalog, MoonCatalog } from "@warpgogol/werkstatt-shared/ontology/cosmic";
import type {
  CheckResult,
  Diagnostic,
  KernelCommandInput,
  ...
  for (const filePath of files) {
    ...
    for (const hit of scanForCosmicLiterals(text, catalogByName)) {
      diagnostics.push({
        ...
        fixHint:
          `Replace the literal with a lookup against @warpgogol/werkstatt-shared/ontology/archetypes (e.g. roleByCosmicName["${hit.name}"]), ` +
