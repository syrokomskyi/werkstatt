...
`collectFiles` from @warpgogol/werkstatt-shared/share/fs. Prevents the ~52-copy duplication class
  ...
  for (const filePath of files) {
    ...
    for (const line of findUnsuppressedWalkDeclarations(source)) {
      diagnostics.push({
        ...
        line,
        message:
          "Nested recursive readdir walker declared outside the canonical @warpgogol/werkstatt-shared/share/fs module.",
        fixHint:
          'Import { collectFiles } from "@warpgogol/werkstatt-shared/share/fs" instead of declaring a local walk() function. If this walker has a genuinely different contract, add a "fs.walk.lint: allow — <reason>" comment directly above the declaration.',
      });
    }
  }
