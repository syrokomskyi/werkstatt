...
<MODULE_CONTRACT>
<purpose>
  RFC-0230 workspace validator for UI-facing helpers in `@warpgogol/werkstatt-shared/share`.
...
export function scanShareI18nSource(
  ...
  while ((match = stringLiteral.exec(source)) !== null) {
    ...
    violations.push({
      ...
      message:
        rule === "SHARE-I18N-02"
          ? "Hardcoded localized label map in @warpgogol/werkstatt-shared/share must be extracted or explicitly allowed."
          : "Human-readable string literal in a UI-facing @warpgogol/werkstatt-shared/share helper must be extracted or explicitly allowed.",
