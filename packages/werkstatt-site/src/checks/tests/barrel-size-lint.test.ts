...
describe("barrel.size.lint (RFC-0264)", () => {
  it("BARREL-01: fails (error) when @warpgogol/werkstatt-shared/share's root barrel exceeds the threshold", async () => {
    const { root, pkgDir } = await fixtureWorkspace();
    await writeFile(
      join(pkgDir, "package.json"),
      JSON.stringify({ name: "@warpgogol/werkstatt-shared/share" }),
      "utf8",
    );
