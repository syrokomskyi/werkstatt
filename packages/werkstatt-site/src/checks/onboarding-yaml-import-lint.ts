...
read MUST go through `@warpgogol/werkstatt-shared/share/onboarding-yaml` so the "first doc is
...
<CHANGE_SUMMARY>
  <item>RFC-0082: Introduce the import-lint guard preventing direct YAML.parse of onboarding artifacts outside @warpgogol/werkstatt-shared/share/onboarding-yaml.</item>
</CHANGE_SUMMARY>
*/
...
const FORBIDDEN_PATTERNS: Array<{ pattern: RegExp; message: string }> = [
  {
    pattern: /from\s+["']yaml["']/,
    message:
      'RFC-0082: direct `from "yaml"` import in an RFC-0076 onboarding-artifact reader. Use `@warpgogol/werkstatt-shared/share/onboarding-yaml` instead.',
  },
  {
  ...
  },
  {
    ...
    message:
      "RFC-0082: aliased `parse as parseYaml` import from the `yaml` package in an RFC-0076 onboarding-artifact reader. Use `@warpgogol/werkstatt-shared/share/onboarding-yaml`.",
  },
];
