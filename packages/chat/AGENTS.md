...
| --- | --- | --- |
| `@gogol/chat` | `src/index.ts` | Server/build-time barrel: port types, config schema, adapter catalog, adapter metadata |
| `@gogol/chat/port` | `src/port.ts` | `ChatWidgetAdapter` interface, `ChatWidgetLoadResult`, `ChatWidgetOpenResult`, `ChatWidgetConfig`, `ChatWidgetConfigSchema`, `CHAT_CONFIG_SCRIPT_ID`, closed `CHAT_ADAPTER_IDS` catalog |
| `@gogol/chat/client` | `src/client.ts` | `bindChatLauncher()` — client-side click-to-load bootstrap |
...
- Do NOT add a vendor `<script>` or `<iframe>` to any `.astro` file. The launcher is rendered by `chat-widget-section.astro` in `@gogol/ui`; the vendor loads lazily via the adapter.
...
| Package                           | Role                             |
| --------------------------------- | -------------------------------- |
| `@gogol/chat-adapter-uchat`       | UChat implementation             |
| `@gogol/chat-adapter-null`        | No-op adapter (safe default)     |
| `@gogol/ui` (chat-widget section) | Renders the first-party launcher |
