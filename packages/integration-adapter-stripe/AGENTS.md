# `@warpgogol/integration-adapter-stripe` — Agent Guide
...
- **Do not resolve the Organization here.** The mapping carries `stripeCustomerId`; the CRM-buffer destination (`@warpgogol/integration-adapter-supabase-crm`, RFC-0190) resolves Person → Deal → Org. The wrapper resolves `needsSubscriptionLookup` by fetching subscription metadata from Stripe when `stripeSecretKey` is provided.
...
`@warpgogol/ui` → `packages/ui/src/integration-routes/stripe-webhook.api.ts` exposes `/api/stripe-webhook`. The mapped `IntegrationEvent` then flows through the EU-resident delivery substrate (QStash + Redis, RFC-0181) like any other source.
...
| `@warpgogol/integration-adapter-supabase-crm` | CRM-buffer destination that resolves the Organization graph and queues Pipedrive sync |
