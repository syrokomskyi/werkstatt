...
export async function emitPassport(options: PassportEmitOptions): Promise<PassportJson> {
  ...
  // Step 5: Run pipeline (hash, sign, assemble passport)
  const ctx: PipelineContext = {
    ...
    buildDurationMs,
    builder,
    starMapDepth: options.starMapDepth ?? 3,
  };
  const { passport, nebulaScore } = await runPipeline(ctx);
  ...
  // Step 7: Render + write star map SVG
  ...
  // Step 8: Heartbeat (never fails build)
