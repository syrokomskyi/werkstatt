...
- `validateSchemaOrgPrices`, `buildCanonicalPriceSet` — Schema.org price validation: ensures only canonical source-currency prices appear in Schema.org output (RFC-0745, exported from `@warpgogol/pbp/compiler`)
...
| `@warpgogol/pbp/compiler` | `src/compiler/index.ts` | `compilePbpProfile` — 14-phase compiler pipeline (discover → parse → validate → index → locale → references → profile → overlays → derivations → semantic → buyer-view → projection → snapshot → publication). Also exports `materializeDerivedPrices` (RFC-0740), `validateSchemaOrgPrices` and `buildCanonicalPriceSet` (RFC-0745). Phase 12 now validates Schema.org prices via `validateSchemaOrgPrices` to block derived price leakage. |
