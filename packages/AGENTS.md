...
| `fingerprint` | RFC-0364 semantic fingerprint package and hash governance. Two entry points: `@warpgogol/fingerprint` (primitives — `byteHash`, `byteHashFile`, `stableStringify`, `stableJsonHash`, no parser deps) and `@warpgogol/fingerprint/semantic` (`fingerprintFile`, `fingerprintTree` with parser-backed normalizers for TypeScript, Astro, CSS, JSON, JSONC, YAML, Markdown). All project hashes use this package; no ad hoc hashing helpers outside it. |
