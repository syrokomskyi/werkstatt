...
| `werkstatt-godot` | Werkstatt Godot plugin — Godot 4.x + C# stack. Implements `werkstatt/plugin@1` with `profileId: "godot-csharp"`. Provides Godot path conventions, scene/gitignore/secret/project-config/scene-reference/csproj/resource validators, dotnet build hook with Godot export, dev server and test commands via dev module, itch.io (multi-platform) and GitHub Releases deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces GODOT-01..07 invariants. Bundles godot-feature, godot-scene-review, and godot-debug skills. |
