...
| `werkstatt-godot` | Werkstatt Godot plugin — Godot 4.x + C# stack. Implements `werkstatt/plugin@1` with `profileId: "godot-csharp"`. Provides Godot path conventions, 11 validators (scene, gitignore, secret, project-config, scene-reference, csproj, resource, script, export-presets, uid, nuget), dotnet build hook with Godot export, dev server, test, smoke test, and context generate commands via dev module, itch.io (multi-platform) and GitHub Releases deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces GODOT-01..11 invariants. Bundles godot-feature, godot-scene-review, and godot-debug skills. |
