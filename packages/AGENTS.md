...
- **Delete test re-export shims when extracting code to a new package.** When source files move from package A to package B (e.g. `werkstatt-site` → `werkstatt-shared`), re-export shims like `export * from "@warpgogol/werkstatt-shared/..."` in A's test files cause vitest to run the tests in both packages. Tests with side effects (network calls, timeouts, real I/O) pass in B but fail or time out in A. Delete the shim file entirely — do not keep it for backward compatibility. The tests live in the new package and run there only.
...
| --- | --- |
| `werkstatt` | RFC-0769/0772: Werkstatt engine — stack-agnostic lifecycle platform. Consolidates kernel runtime, missions, mirrors (Sternsystem), releases, Leitstand, Bordbuch, Notausgang, artifact store, evidence, fingerprint, integrity, observability, agent-gate, changelog, and operations schemas. Plugin contract (`werkstatt/plugin@1`) and registry in `src/plugin-contract.ts` and `src/plugin-registry.ts`. `werkstatt.autonomy.validate` enforces DNA-64 (no stack plugin imports). `werkstatt.shared.validate` enforces RFC-0868 shared/site boundary. See `packages/werkstatt/AGENTS.md` for entry points. |
| `werkstatt-shared` | RFC-0868: Stack-agnostic shared infrastructure extracted from `werkstatt-site`. Owns checks, integration, ontology, passport, share, and surface domains that are consumed by both the engine and site plugin. MUST NOT import from `werkstatt-site` — enforced by `werkstatt.shared.validate`. Published to NPM as `@warpgogol/werkstatt-shared`. |
...
| `werkstatt-phaser-game` | RFC-0777: Werkstatt Phaser plugin — Phaser turborepo stack. Implements `werkstatt/plugin@1` with `profileId: "phaser-turborepo"`. Provides Phaser path conventions, phaser validators, Vite build hook, GitHub Pages and Cloudflare Pages deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces PHASER-01..04 invariants. |
| `werkstatt-godot-game` | Werkstatt Godot plugin — Godot 4.x + C# stack. Implements `werkstatt/plugin@1` with `profileId: "godot-csharp"`. Provides Godot path conventions, 12 validators, dotnet build hook with Godot export, dev server, test, smoke test, context generate, playtest, and screenshot commands via dev module, itch.io (multi-platform) and GitHub Releases deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces GODOT-01..12 invariants. Bundles godot-feature, godot-scene-review, godot-debug, and godot-mcp-bridge skills. |
| `werkstatt-knowledge` | RFC-0894: Werkstatt Knowledge plugin — evidence-backed knowledge systems stack. Implements `werkstatt/plugin@1` with `profileId: "knowledge-typescript-turborepo"`. Provides knowledge path conventions, 5 kernel modules (source, core, extract, materialize, release) with 23 command stubs, 5 lifecycle hooks (materialize, build, checkGate, releaseEvidence, scaffoldProject), and KNO-001..028 stack invariants. No deploy adapters in v1. See `packages/werkstatt-knowledge/AGENTS.md` for details. |
...
- **Test temp directories:** Unit tests that create temp directories (via `mkdtemp`/`mkdtempSync`) MUST use `os.tmpdir()` as the base directory, NOT `process.cwd()`. Using `process.cwd()` places temp dirs inside the package directory when vitest runs from that location, and `systems-cache` siblings (created via `join(tmpDir, "..", "systems-cache")`) are not cleaned up by `afterEach` because only the `tmp-*` dir itself is removed. Use the `tmp-*` naming pattern (e.g. `tmp-leitstand-XXXX-`, `tmp-close-evidence-XXXX-`). These directories are gitignored via `tmp-*/` in `.gitignore`. Agents MUST clean up `tmp-*` directories they create during a session — the session-end workflow automates this, but agents should also clean up manually if a session is not formally closed.
