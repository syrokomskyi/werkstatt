...
| `werkstatt-godot` | Werkstatt Godot plugin — Godot 4.x + C# stack. Implements `werkstatt/plugin@1` with `profileId: "godot-csharp"`. Provides Godot path conventions, scene/gitignore/secret/project-config validators, dotnet build hook, itch.io and GitHub Releases deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces GODOT-01..04 invariants. Bundles godot-feature, godot-scene-review, and godot-debug skills. |
