...
| `werkstatt-godot` | Werkstatt Godot plugin — Godot 4.x + C# stack. Implements `werkstatt/plugin@1` with `profileId: "godot-csharp"`. Provides Godot path conventions, 12 validators (scene, gitignore, secret, project-config, scene-reference, csproj, resource, script, export-presets, uid, nuget, addon), dotnet build hook with Godot export, dev server, test, smoke test, context generate, playtest, and screenshot commands via dev module, itch.io (multi-platform) and GitHub Releases deploy adapters, `scaffoldProject` hook, and `releaseEvidence` hook. Enforces GODOT-01..12 invariants. Bundles godot-feature, godot-scene-review, godot-debug, and godot-mcp-bridge skills. |
