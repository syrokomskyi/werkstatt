...
launcher to the click-to-load loader (@gogol/chat/client). Nothing third-party loads until the
...
<CHANGE_SUMMARY>
  ...
  <item>RFC-0175 fix: own the adapter loader map here (the host) so @gogol/chat has no adapter
        dependency — removes the build:check workspace cycle.</item>
</CHANGE_SUMMARY>
*/

import { bindChatLauncher, type ChatAdapterLoaders } from "@gogol/chat/client";
...
// CHAT_ADAPTER_IDS (@gogol/chat). Nothing third-party loads until a loader is invoked
// on user activation (click-to-load).
const ADAPTER_LOADERS: ChatAdapterLoaders = {
  null: () => import("@gogol/chat-adapter-null"),
  uchat: () => import("@gogol/chat-adapter-uchat"),
};
