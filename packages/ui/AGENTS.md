...
- **RFC-0766: Prose content supports price markers.** Prose files (`src/content/prose/{lang}/*.md`) rendered through `prose-pipeline.ts` now resolve `{price:offering-id:chargeRef}` markers. The pipeline uses `renderPriceDisplayHtml` (not the `CurrencyAwarePriceDisplay` Astro component) because prose is rendered as HTML strings via micromark. The HTML uses `<span>` elements (not `<div>`) to remain valid HTML inside `<p>` elements. Markers inside `<code>` and `<pre>` blocks are skipped. See RFC-0766 for details.
