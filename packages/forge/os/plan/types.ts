/*
<MODULE_CONTRACT>
<purpose>
Type definitions for the plan archive domain — plan file discovery, parsing,
and archive result shapes.
</purpose>
<non-goals>
  <item>Do not define archive handler logic here — that lives in handlers/archive.ts.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>RFC-0521: initial plan archive types.</item>
</CHANGE_SUMMARY>
*/

import type { ArchiveMove, ArchiveSkip } from "../rfc/handlers/archive.ts";

export const PLAN_DIR = "docs/plans";
export const PLAN_FILE_PATTERN = /^plan-rfc-\d{4}-/;

export interface PlanArchiveResult {
  command: "plan.archive";
  status: "ok";
  moved: ArchiveMove[];
  skipped: ArchiveSkip[];
  dryRun: boolean;
}

export type { ArchiveMove as PlanArchiveMove, ArchiveSkip as PlanArchiveSkip };
