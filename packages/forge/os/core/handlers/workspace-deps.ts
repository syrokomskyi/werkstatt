/*
<MODULE_CONTRACT>
<purpose>Lazy dynamic-import bridge for @warpgogol/werkstatt/fingerprint and @warpgogol/werkstatt-site/share/fs. These workspace packages are not published to npm — when forge is installed standalone from npm, they are unavailable and handlers that depend on them must degrade gracefully.</purpose>
<non-goals>
...
export async function loadWorkspaceDeps(): Promise<WorkspaceDeps> {
  ...
  try {
    ...
    const shareMod = await import("@warpgogol/werkstatt-site/share/fs");
    // @ts-ignore — workspace dep, may not be installed when forge is used standalone from npm
    ...
    loadError =
      `@warpgogol/werkstatt/fingerprint and @warpgogol/werkstatt-site/share are required for this command but not installed. ` +
