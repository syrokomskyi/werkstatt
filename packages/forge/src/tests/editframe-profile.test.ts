/*
<MODULE_CONTRACT>
<purpose>Unit tests for the editframe-html stack profile — verifies domain fields, terminology, artifacts, workspaceTypes, invariants, and detect markers.</purpose>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
...
test("loadStackProfile succeeds on editframe-html.yaml", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  expect(profile.id).toBe("editframe-html");
});

test("stackProfileSchema.safeParse succeeds on editframe-html profile", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  const result = stackProfileSchema.safeParse(profile);
  expect(result.success).toBe(true);
});

test("editframe-html profile declares domain: video", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  expect(profile.domain).toBe("video");
});

test("editframe-html profile declares register: creative", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  expect(profile.register).toBe("creative");
});

test("editframe-html profile declares terminology map with artifact, module, operator", () => {
  const profile = loadStackProfile(PROFILE_PATH);
...
test("editframe-html profile declares artifacts with composition extensions", () => {
  const profile = loadStackProfile(PROFILE_PATH);
...
test("editframe-html profile declares workspaceTypes with composition detection markers", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  ...
  expect(compositionType?.detect.glob).toBe("*.html");
  expect(compositionType?.detect.contains).toBe("ef-timegroup");
  expect(compositionType?.detect.packageJsonDep).toBe("@editframe/cli");
});

test("editframe-html profile declares detect.anyOf with editframe.config.*", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  expect(profile.detect.anyOf).toContain("editframe.config.*");
});

test("editframe-html profile declares at least 3 VIDEO-* invariants", () => {
  const profile = loadStackProfile(PROFILE_PATH);
...
test("editframe-html profile includes first workspace template with sample HTML composition", () => {
  const profile = loadStackProfile(PROFILE_PATH);
  expect(profile.firstWorkspace).toBeDefined();
  expect(profile.firstWorkspace?.path).toBe("compositions/my-first-video");
  const htmlFile = profile.firstWorkspace?.files.find((f) => f.path === "composition.html");
  expect(htmlFile).toBeDefined();
  expect(htmlFile?.content).toContain("ef-timegroup");
});

test("editframe-html profile includes @warpgogol/forge in install steps", () => {
  const profile = loadStackProfile(PROFILE_PATH);
