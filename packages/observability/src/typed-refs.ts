...
@ai-invariant: METRIC_REFS keys must match WGOGOL_METRIC_REGISTRY names exactly — enforced by compile-time type assertion below.
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
...
import { WGOGOL_METRIC_REGISTRY } from "./metric-registry.ts";
...
export const METRIC_REFS = {
  wgogol_factory_smoke_total: defineCounter("wgogol_factory_smoke_total", [] as const),
  wgogol_factory_command_runs_total: defineCounter("wgogol_factory_command_runs_total", [
    "command",
    "status",
    "site_id",
  ] as const),
  wgogol_factory_command_duration_seconds: defineHistogram(
    "wgogol_factory_command_duration_seconds",
    ["command", "site_id"] as const,
  ),
  wgogol_factory_diagnostics_total: defineCounter("wgogol_factory_diagnostics_total", [
    "command",
    "severity",
    "site_id",
  ] as const),
  wgogol_probe_up: defineGauge("wgogol_probe_up", ["site_id", "route"] as const),
  wgogol_probe_ttfb_seconds: defineGauge("wgogol_probe_ttfb_seconds", [
    "site_id",
    "route",
  ] as const),
  wgogol_probe_http_status_class_total: defineCounter("wgogol_probe_http_status_class_total", [
    "site_id",
    "route",
    "status_class",
  ] as const),
  wgogol_probe_content_ok: defineGauge("wgogol_probe_content_ok", ["site_id", "route"] as const),
  wgogol_probe_cert_expiry_days: defineGauge("wgogol_probe_cert_expiry_days", ["site_id"] as const),
  wgogol_probe_deep_ok: defineGauge("wgogol_probe_deep_ok", ["site_id"] as const),
  wgogol_delivery_requests_total: defineCounter("wgogol_delivery_requests_total", [
    "site_id",
    "cache_status",
    "status_class",
  ] as const),
  wgogol_delivery_bytes_total: defineCounter("wgogol_delivery_bytes_total", ["site_id"] as const),
  wgogol_workers_requests_total: defineCounter("wgogol_workers_requests_total", [
    "site_id",
  ] as const),
  wgogol_workers_errors_total: defineCounter("wgogol_workers_errors_total", ["site_id"] as const),
} as const;

// Compile-time assertion: every METRIC_REFS key must be a declared registry metric name.
// If a metric is added to METRIC_REFS but not to WGOGOL_METRIC_REGISTRY (or vice versa),
// this type expression produces a compile error.
type _RegistryNames = (typeof WGOGOL_METRIC_REGISTRY)[number]["name"];
