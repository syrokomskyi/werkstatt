# @warpgogol/observability — Agent Guide
...
| `metric-registry.ts` | Closed `WARPGOGOL_METRIC_REGISTRY` — every metric emitted anywhere MUST be declared here. `findMetricSpec` for kind lookup. |
...
```ts
import { METRIC_REFS } from "@warpgogol/observability";
METRIC_REFS.warpgogol_factory_command_runs_total.add(pusher, 1, { command: "build.check", status: "pass" });
```

Dynamic metric names (e.g. from poll transforms) use `findMetricSpec` for kind lookup:

```ts
import { findMetricSpec } from "@warpgogol/observability";
...
- Compile-time type assertion in `typed-refs.ts` ensures `METRIC_REFS` keys ⊆ `WARPGOGOL_METRIC_REGISTRY` names.

## Adding a new metric

1. Add the spec to `WARPGOGOL_METRIC_REGISTRY` in `metric-registry.ts` (name, kind, help, labelKeys, unit, buckets).
